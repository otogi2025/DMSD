...
> ## ⚠️ 中文铁律（2026-05-03/04 itsuki 三次强调被骂）
>
> **CC 写的所有「内部 / 给 itsuki 看的」内容 100% 用中文。日语 = 违反。**
>
> ### 1. 代码注释（铁律）
>
> 适用范围（无例外）：
> - **所有语言**：Python `#` / Swift `//` 和 `///` doc 注释 / Kotlin `//` 和 `/** */` / TS `//` / SQL `--` / Markdown 内 code block 注释
> - **所有文件类型**：app 代码 / migration / 测试 / shell 脚本 / config 文件
>
> ### 2. 规范文档 / 设计文档（铁律 — 2026-05-04 第三次强调被骂时加上）
>
> 适用范围：
> - `02_design/system_features.md` 全部新加 / 改写的章节、表格、bullet
> - `03_dev/{backend,student_ios,teacher_web}/*_DESIGN_LOG.md` 全部
> - `00_admin/` 下 itsuki 看的所有文档（WIP / TODO / 同步点清单 / 文件结构指南 等）
> - `05_logs/raw/*.md` AC 素材 dump（itsuki 看的）
> - `01_specs/` 全部 spec 文档
>
> **强约束**：itsuki 是这些文档的**唯一长期读者**（教授看的志望理由书等是另一回事 — 那个用日语，itsuki 自己写）。规范文档用日语写 = itsuki 读得慢、容易看漏关键细节。
>
> ### 3. 边界（不要混淆 — UI 字符串和给最终用户看的文案保持日语）
>
> - **保持日语 ✅**：UI 字符串（`Text("お知らせ")` / 邮件正文 / push 文案 / spec 里固定文案 § 引用 e.g. "コードが正しくない…"）
> - **保持日语 ✅**：教授 / 老师看的产品介绍文档（Tomoshibi 介绍页等）
> - **必须中文 ❌→✅**：上述周围的注释 / 描述 / 表格 column 名 / 状态标记等
>
> ### 4. 判定关键词（出现这些 = 漂移，立即改）
>
> ひらがな / カタカナ / 日本語特有汉字组合：
...
> - 高频功能词：処理 / 機能 / 設定 / 実装 / 検証 / 生成 / 取得 / 登録 / 削除 / 更新 / 表示 / 一覧 / 必須 / 任意 / 既存 / 新規 / 投稿 / 詳細 / 既読 / 返信 / 矩陣
> - 数量 / 因果：以内 / 以上 / 以下 / 程度 / 等 / など / ため / ので / から
>
> ### 5. CC 自检
>
> 每写完一段（10-30 行 / 一个表格 / 一个章节）→ 扫一下中文铁律覆盖的部分有没有日语词 → 有就当场改、不要批量等到结束。
>
> 详见 memory `feedback_code_comments_chinese_strict.md`。
...
## 会话开始：只读 WIP.md（其他长文件按需读）

**启动时 CC 只读一个文件**（2026-05-04 itsuki 拍板修订）：

- **`00_admin/WIP.md` 全文**（短，~150 行）— 当前版本 / 当下焦点 / 最近 5 次会话做了什么 / 多会话占用 / 阻塞项

读完 WIP + CLAUDE.md（本文件已自动注入）→ CC 已经知道「项目是干嘛 + itsuki 是谁 + 当下状态」三件事。**给 itsuki 报告状态，等指令，不主动催进度**。

### 按需读（CC 不主动读，只在 itsuki 触发对应场景时读）

**`00_admin/项目文件总览.md`**（~600 行）— repo 里每个文件干嘛的 / 还在用没 / AC 价值。**只在「跟文件相关」的问题出现时读**：
- itsuki 问「某文件干嘛用的」/「某文件在哪」/「找一下某某文件」
- 需要搞清楚仓库结构 / 找某类文件
- **替代 `grep` / `find` / 命令行查找** — CC 直接翻这份总览找答案，不用命令行乱搜
- itsuki 没问文件相关的事就不主动读

**维护铁律**：CC 每次创建新文件 / 删文件 / 大幅改动文件作用 → 当场同步更新本文件对应章节。**不再开新的"审查报告"文件**（`2026-XX-XX_xxx_审查.md` 模式弃用）— 项目文件总览是单源真值。

**`00_admin/TODO.md`**（~660 行）— 所有未完成事项的完整 backlog。**只在 itsuki 主动问「还有什么没做」/「下一步该做什么」时读**：
- itsuki 没问就不主动读
- 不主动催进度 / 不主动列待办给她看
- 会话中新增的 backlog 写到 TODO.md（不写 WIP），但写完不需要主动让她过目

### WIP vs TODO 分工铁律

WIP = 当下书签（短小、5 次会话上限）；TODO = 完整未完成 backlog（真值）。**WIP 绝不复述 TODO 的内容**。CC 可直写 WIP / 可加任务和标完成到 TODO；`progress_overview.md` 只起草等 itsuki 确认。

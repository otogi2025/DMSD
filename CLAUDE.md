...
> ## ⚠️ 代码注释强制规则（铁律 — 2026-05-03 itsuki 第一次强调，2026-05-04 第二次强调被骂）
>
> **代码注释 100% 只用中文。日语 = 违反。**
>
> 适用范围（无例外）：
> - **所有语言**：Python `#` / Swift `//` 和 `///` doc 注释 / Kotlin `//` 和 `/** */` / TS `//` / SQL `--` / Markdown 内的 code block 注释
> - **所有文件类型**：app 代码 / migration / spec 内嵌示例代码 / 测试 / shell 脚本 / config 文件
> - **不管周围 UI / 文本是不是日语**：做日语 UI 功能时尤其容易日语漂移，必须警觉
> - **不管 spec 是不是日语**：spec 用日语写不代表注释也用日语，必须翻译成中文
>
> **判定**：注释里出现 ひらがな / カタカナ / 日本語特有汉字组合（「の」「を」「が」「は」「に」「で」/ 「の場合」「等」「である」/ 「設定」「実装」「処理」「機能」等高频词）= 漂移，必须改。
>
> **UI 字符串和注释的边界（不要混淆）**：
> - UI 字符串（`Text("お知らせ")` / `Button("送信")` / 邮件正文 / push 通知文案）= 保持日语 ✅
> - 这些 UI 字符串旁边的注释 = 必须中文 ❌「// 送信ボタン」→ ✅「// 提交按钮」
> - schema 字段 description / OpenAPI 描述 = 中文（除非明确给日本人用户看的 i18n 字符串资源）
>
> **CC 写代码时的自检**：每写完一段代码（10-30 行），扫一下注释里有没有上面的日语词；有就当场改，不要批量等到结束。
>

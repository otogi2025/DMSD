...
- **教师权限模型** → **本段「按职责勾选」旧模型已作废（2026-06-11）**，被「权限组 + 按功能两级开关」新模型取代。
  ...
  **寮边界 2026-07-17 拍板改「分角色」**（高层权限组跨寮 / 普通老师本寮，取代 6-13 全放开与 6-18 选寮过滤）→ 方向与实装范围见 `design/teacher_permission_v1.md §13`，第二波实装。
...
> - 提交后 → 显示承认状态(#5)
> - 通知 = **邮件**(R1)
> - **教師代録(2026-06-04 杭田五-3「教師用は当日入力可」追加)**: 上面 #1「不能代填」限的是**学生不能替别人填**;**寮務系教師(寮務部長 / 寮務課長 / 寮監 / 寮務一般教師 / 管理係 5 角色)可代学生提出**,且代録放宽到**当日も可**(学生侧仍禁当日 #3)。代録走 `POST /applications/by-teacher?student_id=`,学生选择器走 `GET /applications/proxy-candidates`(同 5 角色),提出後は学生提出と同じ承认链。**⚠️ 寮过滤 2026-06-18 改「登录时选寮」重开**(部分推翻 6-14 取消,见 decision_log 2026-06-18):选了寮的老师代録/搜学生只限该寮,「申請承認専用」/op 仍可任意寮。老师网页 `ProxyApplicationPage` + 后端实装完了(2026-06-05),iOS / Android 学生端不含此功能(代録是老师专用)。
...
> **2026-07-17 拍板（后端双会话审查逻-中-9 收口）— 并行会签**：chain 是「**谁都要批、谁先批都行**」的会签，不是接力审批。下表箭头（→）只表示纸质实物表上印欄的排列顺序 = 系统里的**显示顺序**（`chain_order` 字段），**不构成审批顺序门禁**——后端不检查前一位是否已批，任何 chain 上的役职随时可批。代码本就如此实装（`chain_order` 仅用于排序显示），本拍板将其确认为正式设计。
|---|---|---|
...
#### 7.2.7 外出申請（当日帰寮の短時間外出）— 事後確認制（itsuki 2026-07-22 拍板 / 原 2026-06-04 単一先生確認）
...
>
> **2026-07-22 二次拍板**：进一步取消「事前审批」语义 —— 学生提交后**立刻生效可以出门**，不需要等老师同意。老师的「確認」是**事后留记录，不是放行开关**。现实依据：外出本来就是低风险的日常行为，让学生站在门口等老师点按钮不符合实际宿舍运作。
...
|---|---|---|
| 过夜 | 是（不在宿舍过夜）| 否（当天回宿舍）|
| 审批 | 多级审批链 4–6 人（§7.2.2）| **事后确认 — 提交那一刻就生效** |
| 「審査」阶段 | 有 | **无** |
| 进度显示 | 节点链 chainDots（§7.2.6）| **2 步：提出 → 先生の記録待ち** |
| 老师不同意时 | 差戻し / 却下 → 出不去 | **却下 = 通知 + 记录**，不强制学生回寮 |

**事后确认制怎么运作（itsuki 2026-07-22 拍板）**:
- 学生提交 = 当场就能出门，不用等老师操作。
- 老师点「確認」只是留记录。学生端状态徽章写 `承認不要`，进度第 2 步写 `先生の記録待ち`，说明文写 `確認は記録のためで、外出は可能です`。
...
- 判定时点 = **按下提交那一瞬间**的分数（提交时 7 分、外出当天涨到 8 分的情况不追溯）。
- 分数口径跟排行榜 / `/me/summary` 走**同一个函数**（当月 + 排除已撤回），不各写各的 SUM。
- 学生端也要把申请入口的卡片本身置灰并显示同一句理由，不只靠服务端弹回。
...
- **2026-07-22 起 `confirmed_*` 三列的语义扩大为「処理した先生 / 処理時刻」** —— `status=approved` 时是确认者、`status=rejected` 时是却下者。外出是单人一次性终态处理，不存在「先确认再却下」，配合 `status` 读没有歧义，故不再另开 `rejected_by_*` 两列。客户端显示时必须按 `status` 分支（「確認 · ○○ 先生」/「却下 · ○○ 先生」）。
...
- 学生端**没有**「应用内通知」表 —— `models.Notification` 是老师通知中心专用的（用 `is_demo` 做演示/生产隔离，已读状态按老师各算一份），学生根本看不到。所以只有上面 2 路。
- 两路各自用 SAVEPOINT 隔离（跟 `applications.py` / `notifications.py` / `study.py` 的兜底同一写法）：通知写失败**不能把已经成立的却下回滚掉**，一路失败也不能把另一路带走。
...
> - **2026-07-17 拍板(后端双会话审查逻-中-3 收口)— 不引入「学期」概念**: 名簿是一条**持续名单**,只随老师按钮重置,不按学期字符串自动切分。现行后端 `StudyRoster` 按 `academic_term` 字符串过滤,字符串切换当天旧名簿全部不再命中、页面静默变空(审查实测口径: 元旦即触发)。晚自习属第二波,实装前按本拍板去掉 `academic_term` 自动切分(改持续名单或仅按年度);上一条「每学期开始前 1 周邮件提醒」相应改为按老师自己的重置节奏,系统不内置学期定义
...
- 鉴权: 上传仅本人 + 仅 pending 状态;下载学生本人 OR 老师。**寮过滤 2026-06-18 改「登录时选寮」重开**(部分推翻 6-14 取消,见 decision_log 2026-06-18):选了寮的老师只能下载/看到该寮学生契約書,「申請承認専用」/op 组仍可任意寮。
...
**寮过滤（2026-08-01 补齐）**：事案 5 个端点（建 / 列表 / 详情 / 编辑 / 软删）此前完全没有寮过滤，男寮当值老师能读到女生事案的全文和真名——上线前审查 H4 修复，判定口径与外出确认 / 契約書 / 代録同款：选了寮的老师只能碰**涉及学生全部在该寮**的事案，否则 403 `FORBIDDEN_DORM`；「申請承認専用」/ op 组仍可任意寮；无涉及学生的事案不属于任何寮、不受限。
...
### 7.11 リクエスト曲管理(音乐功能 — 4-29 itsuki 拍板"留"；2026-07-06 拍板**冷冻**)

> **itsuki 拍板**: 学生发帖功能 + 社区功能整体 — 砍 / 匿名建议 — 砍 / **音乐功能 — 留**
>
> **2026-07-06 冷冻**: 老师网页 CommunityPage 整页未接后端、学生 iOS 仅占位壳 → 老师网页菜单摘除（commit `f66b812`），本节矩阵、后端 songs 路由/表、两端代码全保留原样，实装版本归属以后定。复活时恢复 `Shell.tsx` 菜单一行即可。
...
#### 7.11.3 投稿通報 + 老师删除（2026-07-20 拍板 A 方案 — App Store 审核指南 1.2 UGC 治理）

覆盖三类学生互见投稿：リクエスト曲 / お知らせ返信 / 落とし物（原「通报+累计封禁」体系 6-13 拍板彻底删除后，为上架合规加回的不含封禁最小版）。

| 功能 | 学生 iOS | 老师 Web | 后端 API | 角色 |
|---|---|---|---|---|
| 通報（确认弹窗 + 理由任意）| ✅ 曲詳細/遺失物詳細按钮 + 公告回复长按菜单 | — | `POST /reports`（幂等）✅ | 学生 |
| 通報一覧 + 標処理完 | — | ✅ 「投稿の通報」页（未対応/対応済み筛选）| `GET /reports` / `PATCH /reports/:id` ✅ | 老师 |
| 投稿删除（软删，学生一览即消失）| — | ✅ 通報一覧「投稿を削除」按类型分流 | `DELETE /songs/:id` / `DELETE /lost-found/:id` / 公告回复既有 DELETE ✅ | 老师 |
...
### 7.23 操作履历审计 / 老师操作记录页（後端 + 老师网页，2026-06-16 itsuki 拍板）

老师网页要能查看老师的操作历史，带精确日期时间 + 做了什么操作。涉及後端 + 老师网页两端。

**功能定位**：管理职能。给管理角色一个「操作履歴」页，回溯老师在系统里做过的写操作（谁、什么时刻、做了什么、对什么对象），用于事後审计 / 追责 / 排查误操作。

**记录范围**：全部老师的写操作（POST / PUT / PATCH / DELETE）。学生 / 匿名请求不记。仅记成功（2xx/3xx）的请求。登录登出（带密码）不记。部分操作另有各功能自写的语义级审计行 —— 操作记录页做了去重，同一操作只显示一次。

**做法（自动埋点）**：後端用一层中间件（`AuditLogMiddleware`，挂在请求链最外层）拦截全部老师写请求，自动落库一笔到 `audit_logs` 表。记的内容 = 操作者 + 「请求方法 + 归一化路径」（路径里的学号/对象 id 归一成 `{id}` 便于聚合）+ 脱敏后的请求体（密码/令牌类字段替换成 `***`）+ 状态码。老师无需做任何事、各业务功能无需逐处埋点，写操作即自动留痕。

**谁能看**：仅管理角色。后端只读端点 `GET /api/v1/admin/audit-logs` 挂权限簇「操作履历审计」（`C_AUDIT_LOG`，权限矩阵见 `teacher_permission_v1.md §5` 第 17 簇）—— op / 寮管理者 / 一般宿管（寮務部長·寮務課長·管理係）可查阅；寮監·学習担当（一般宿管+晚自习）与国際交流（申請承認専用）不可见。老师网页「操作履歴」菜单也按此权限对管理角色才显示。

**演示隔离**：操作记录按操作者的演示/真实身份隔离 —— 演示老师只看演示老师的操作、真老师只看真老师的，互不可见（对齐全系统 `is_demo` 横切隔离）。

**附带改动**：登录后默认页统一为「点呼」（原来按角色分流，现所有老师登录都落到点呼页）。itsuki 同一需求里一并拍板。

**各端落地细节**：`BACKEND_DESIGN_LOG.md §3.7.1`（中间件 + 只读端点 + 表改动 + 迁移 `a9b8c7d6e5f4` / `c5d6e7f8a9b0`）/ `WEB_DESIGN_LOG.md §21`（`AuditLogPage` + 导航项 + 默认页）/ `teacher_permission_v1.md §5 §6 §12`（第 17 功能簇 + 端点映射 + 决策记录）。**实装范围（v1）**：後端 + 老师网页。iOS / Android 学生端不涉及。点呼机不涉及。

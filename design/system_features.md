...
##### ⭐ 实物表補完(2026-05-28 — 「届け類.pdf」様式4-1「寮学習欠席届」, CC + codex 双读核对)

实物表显示「学習欠席」实际分**两种类型**(同一张纸上下两栏)。上面 §7.3.5 现有设计对应的是**类型 B**(体調不良 / 特別課題)。**类型 A(オンライン学習申請)现有设计完全没有,需补**:
...
- 实物表注意事项补充: 判断依据 = 当天学校的样子 / 保健室利用 / 发热状况; 学習欠席次数多 → 不许可 or 附条件许可(呼应现有「月度超 3 次」机制)

**审批盖章链(实物表 4 角色, 类型 A / B 共用同一张届)**:
- 寮務課長(杭田)→ 寮務部長(新谷)→ 学習担当(加納)→ 当日担当(仅类型 B 欠席当天有)
- ✅ **itsuki 2026-05-28 拍板「一人审查」**: 数字化后学習欠席届 = 单人审批(学習担当 / 当晚晚自习监督老师), **不做多角色链**。实物纸面的 4 角色盖章是纸面行政流程, 数字化简化为一人批。
...
|---|---|---|---|---|---|
| 学習 NFC 碰 2 次签到（开始 / 结束，itsuki 5-31 删中场）| 〇 NFC tap | — | `POST /study/checkins` ⏳ | 学生 | (V1) |
...
| 功能 | 状态 | 理由 |
|---|---|---|
| **学生掲示板**(投稿 + 阅览 + 通报)| 🚫 **砍**（iOS 代码 2026-06-03 真删落实 — 此前文档砍了代码漂 1 月，寮ウォール WallView）| demo 期间为了"功能完整感"加的,demo 通过后回头看不在核心价值(点呼 + 出寮届 + 学習 + 扣分)里 |
| **社区功能整体** | 🚫 **砍** | 同上 |
| **匿名建议 投稿** | 🚫 **砍** | 同上 |
...
> **背景**: itsuki 2026-05-28 提供宿舍真实纸质申请表「届け類.pdf」(朝日塾中等教育学校 寮)扫描件 9 种。除出寮届三兄弟(§7.2)、学習欠席届(§7.3.5)外, 还有以下 **4 种全新申請**, 现有设计完全没有。CC + codex 双读核对一致。
>
...
>
> ✅ **v1.0 范围(2026-05-28 itsuki 拍板「都进 v1.0」)**: 这 4 种全新申請**全部进 v1.0**。连同出寮届補完(§7.2)+ 学習在线申请(§7.3.5 类型 A)+ 帰国届(様式3-1)一起实装。

...
## 13. 寮边界分角色（2026-07-17 拍板 — 方向 + 跨寮组清单均已定，实装归第二波）
...
**跨寮组清单（2026-07-18 itsuki 拍板，A 方案）** —— 鉴权按权限组判（职位仅显示标签，§11.1），故「高层」落到组：

| 权限组 | 寮边界 | 理由 |
|---|---|---|
| `op` | **跨寮** | 系统最高运维账号，本就无 `assigned_dorm`（§7） |
| `寮管理者` | **跨寮** | 职位默认映射 = 校長 / 寮務部長 / 寮務課長，宿舍高层需跨寮协调 |
| `申請承認専用` | **跨寮**（维持 6-18 特例） | 国際交流职责就是审全校学生的届；收回会卡住女寮学生的审批链 |
| `一般宿管` | 本寮 | 管理係等一线，日常只经手本寮 |
| `一般宿管+晚自习` | 本寮 | 寮監 / 学習担当，一线运营 |

- **本寮层（上表后两组）**：只能查看 / 操作本寮（男寮 = 1·2 寮 / 女寮 = 4 寮）。寮归属改为**服务器侧校验**（按账号登记的 `assigned_dorm`），不再信登录时自报的 `selected_dorm`。
- 决策链：6-13「所有老师看所有学生」（§11.2 寮过滤彻底取消）→ 6-18 部分重开「登录时选寮」→ **7-17 本拍板（分角色）取代前两者**。
- 实装范围（大）：`deps.dorm_units_for_teacher` 按上表分层（跨寮组返全寮 / 本寮组返账号 `assigned_dorm` 对应组）是主改点，现有 24 处 `FORBIDDEN_DORM` 校验多数经它自动生效 + `applications` 补过滤 + `selected_dorm` 服务器校验 + 约 35 个寮边界测试重写。

...
- [ ] **F1（重大·安全）寮边界缺失被权限分级放大**：14 个端点挂了 `require_permission` 但 body 没有寮过滤（`dorm_units_for_teacher`），分寮老师拿到功能权限后可**跨寮查看/写**。CC 核实：`admin_accounts` 的 `/students` `password-reset` `unlock`（103/196/261）= **真实新增暴露**（改动前 ADMIN_ROLES 只 3 职位，改动后所有 M 组含分寮 寮監/寮務一般教師可达，且无寮过滤）；`incidents`(82/134/159/184/246) = 既有问题（改动前 `_INCIDENT_ROLES` 已含分寮角色）；`rollcall`(536/625)`study`(573/639/675)`applications`(790) 待逐个核实是既有还是放大。**设计 §5 已写明「寮过滤与权限正交叠加、不变」→ 这些端点违背设计自身原则。** 待拍板：v1.0 前修（给 14 端点 body 补寮过滤 + 定 404/403 语义）还是推 v1.1。
- [ ] **F3（重大→实为设计取舍）建账号允许 permission_group=NULL → 回退职位**：`create_teacher`(teachers.py:235) 故意留了「省略则按职位回退」的安全网。codex 建议强制必填（含邀请表持久化）。待拍板：强制必填 vs 保留回退网。
- [ ] **F4（重大→= 待拍板②）delete_teacher 防删最后管理员按职位标签数**：`teachers.py:319` 用 `TEACHER_ADMIN_ROLES`(role) 计数。role 退化后可能误判。待拍板：改为按 effective_group 对 `C_TEACHER_ACCOUNT` 达 MANAGE 计数。
- [ ] **F5（重大·产品）学習担当 回退/回填映射到 申請承認専用**：`permissions.py:145` + 迁移回填。但「学習担当」名义负责学习/晚自习，而 申請承認専用 对晚自习只有 VIEW → 迁移后这类既有账号可能被锁出晚自习管理。待拍板（取决于学習担当实际职责）：若真管晚自习应回填 `一般宿管+晚自习`。

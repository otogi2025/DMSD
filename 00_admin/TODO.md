...
> 判定标准：不做完就不能在宿舍正式上线 — 影响真实学生/老师使用、或安全问题、或数据会错。各段保留原标题，背景看各段引言。

## 🔍 2026-06-10 iOS 全量审查 + 前后端对齐（26 分片多代理 + 对抗验证 — 额度中断提前收口）

> **背景**：itsuki 要「审查所有 iOS 代码 + 前端后端对齐」。13 个代码审查分片（全量 22543 行 Swift）+ 13 个接口域对齐代理，一线报 233 条去重发现 → 逐条对抗验证（重大 3 票多数决）。**额度两次用尽 itsuki 喊停**：146 条完成验证（确认 138 / 推翻 8，误报率约 5%）、**86 条没来得及验证**（大概率为真但未核实，标 ⚠️）。已跟 §🔍 6-09 / §📱 上线缺口去重，同根因多处合并。按 itsuki 6-09 拍板：**只记录不当场修**。
> 原始数据（含每条验证理由）：`05_logs/raw/2026-06-10_iOS全量审查_原始发现数据.json`。验证缓存还在，下次想补验 86 条可续跑。

### 🔴 重大 — 已验证确认（5 组）— ✅ 全做完（2026-06-11，双 scheme BUILD SUCCEEDED）

- [x] **R-1 点呼显示链整条是假的（生产版）** — ✅ 2026-06-11 走方案 B：新建后端学生端 `GET /rollcall/me/today`（commit `8cdff97`）→ iOS 接上真实时间窗 + 后端判定（commit `20776b6`/`9f92d00`）。①②③ 三处假数据全接真。
- [x] **R-2 点呼中/欠席状态 UI 是死件** — ✅ 2026-06-11 AppStore `refreshRollStateFromSessions` 时间窗状态机驱动 rollState（commit `20776b6`），随 R-1 同批做完。
- [x] **R-3 三个点呼上报弹窗无防连点** — ✅ 2026-06-11 三 Sheet 照 RenewStudentNoSheet 范本加 `submitting` 守卫（commit `d3d0439`）。
- [x] **R-4 登录锁定 iOS 与后端两套口径** — ✅ 2026-06-11 生产版 401 只提示凭证错不走假倒计时；新增 catch server(423)/(403) 显后端真锁/停用文案；演示版保留本地演出（commit `7841f5a`）。
- [x] **R-5 体調報告履歴整页显示假病历** — ✅ 2026-06-11 `RollCallReportsAPI.listMine()` 接 `GET /rollcall/reports/mine`，MyHealthView 双轨 + 三态，生产版不再显 SEED.health（commit `e4078c5`）。

### ⚠️ 重大 — 未验证（额度中断没跑完验证，修之前先逐条核实代码；合并后 22 条）

**注册/登录簇**
- [ ] 注册密码长度三层不一致：UI 提示 8+ / 客户端校验放行 6 位 / 后端要求 8+，6-7 位密码最后一步必失败且只见笼统「入力エラー」（`NetworkModels.swift:295-297` + `AuthStubs.swift:1329-1335`）【对齐类】
- [ ] 注册完成页生产分支直读 SEED.user，显示假名假账号番号（`AuthStubs.swift:1471-1489`）
- [ ] 房号字母前缀两处衍生（同根因的已验证条见 Y-4）：输「M205」寮号推断错（`AppStore.swift:47-51`）；字母开头房号直发后端必 422（`AuthStubs.swift:896-905`）【对齐类】
- [ ] 登录页「メール」tab 生产可见但后端无邮箱登录端点，永远拒绝登录（`AuthStubs.swift:1686-1729`；配套文案条已验证见 Y-37）【对齐类】
- [ ] 注册 Step1「写真を選択」生产死按钮（`AuthStubs.swift:729-746`）

**申请表单簇**
- [ ] StayForm 出寮届提交无防连点：双击创建两份申请、双发审批邮件（`ApplyStubs.swift:963-965`）
- [ ] 「下書き保存」纯假动作：弹成功 toast 什么都不存、「下書き」tab 生产永远空、用户表单数据丢失（`ApplyStubs.swift:841-843,1833-1847`）
- [ ] 修改届可把宿泊先清成空数组绕过后端「至少 1 件」约束 + 多条滞在先被压成 1 条丢地址电话（`StayListStubs.swift:1215-1218,1539-1543`）【对齐类】
- [ ] 行事企画/冷蔵庫/物品所持三表单无防连点，重复点击写入重复申请（`DormLifeForms.swift:147-153,404-411,630-638`）
- [ ] 修繕/来訪/代理受取提交后进黑洞：无一览/详情/撤回入口（`ApplyStubs.swift:124-131,1946-2002`；其中「后端 GET /misc-requests/mine 有、iOS 没接」这半已验证 → Y-10）
- [ ] 来訪者表单来訪日默认写死过去日期 2026-04-25，可原样提交、后端不拦（`ApplyStubs.swift:1571-1572,1714-1718`）
- [ ] 列表加载失败的错误文案算出来但从不显示，网络失败渲染成「申請はありません」假空态——已知「6 列表假空态」范围再扩大：StayList（`StayListStubs.swift:401,441-447,494-501`）+ DormLife 三列表（`DormLifeForms.swift:218-256`）+ オンライン学習一覧（`StudyOnlineForm.swift:364-402`）+ 宅配页（`CommunityStubs.swift:319-339`）

**社区/行事簇**
- [ ] 遗失物投稿整链路死件：投稿页无任何导航入口 + 后端只收学生令牌 → 生产没有任何端能写遗失物表、列表永远空（`CommunityStubs.swift:567,706-848`）【对齐类】
- [ ] MusicNewView/LostNewView 投稿无防连点重复 POST（`CommunityStubs.swift:1242-1271,818-848`）
- [ ] 活動カレンダー整页（EventsView）生产读 SEED 假行事 + 日历硬编码 2026 年 4/5 月（`CommunityStubs.swift:1446-1659`）
- [ ] 行事详情生产无处可看（description/结束时刻全程不可达、行尾箭头死按钮）——比已知 #2（EventDetailView 下标越界）范围更大（`ScheduleStubs.swift:119-125,280-308,394-401`）
- [ ] 巴士便 visible_to（性别/寮限定可见范围）两侧都不执行：iOS 映射后不过滤、后端也不过滤——女生能看到男生专用便（`BusListStubs.swift:144,183-195` + `bus_routes.py:52-78`）【对齐类】

**学習（晚自习）簇**
- [ ] 整链路生产闭死：studyState 无数据源恒 idle + isStudyTarget 硬编 false（被老师列入名簿的学生 app 里永远「学習対象外」，且该标记无任何传输路径）+ 学習履歴丢弃后端已返回的 study_checkins（`AppStore.swift:291,595-603,770-778` + `MyPageStubs.swift:2471`）【对齐类】

**推送/账号簇**
- [ ] 生产登出不清 pushNotifications：前一个用户的推送（含減点/申請隐私）残留给下一个登录者（`AppStore.swift:102-122`）
- [ ] 登出不撤销设备令牌、后端无撤销端点：已登出设备继续收旧用户隐私推送（`device_tokens.py:26-35`）【对齐类】
- [ ] 后端 send_push 零业务调用点：设备令牌收了存了，但没有任何业务事件触发推送——推送链路后端侧也是断的（`app/services/push.py:114-122`）【对齐类】
- [ ] 個人情報页生产显示「生年月日: (0 歳)」：/me 不含生日但行照渲染（`MyPageStubs.swift:648`）【对齐类】

**演示构建反向漏洞 + 组件**
- [ ] 演示构建直打真后端（跟「生产读 SEED」相反方向的同类病）：StudyOnlineForm 全文件 / submitStudyLeave / DormLifeForms 6 调用点都没有 #if DEMO 分支（`StudyOnlineForm.swift:286-330` + `AppStore.swift:630-667` + `DormLifeForms.swift:243-256`）
- [ ] PageHeader 长按弹面包屑松手时返回按钮动作同时触发，导航双触发（`PageHeader.swift:28-52`）

### 修复顺序建议（等 itsuki 拍板）

1. **先修 R-1/R-2（点呼显示链）+ 注册/登录簇** — 点呼是系统核心卖点，上线第一晚就暴露；注册是新生第一接触点。
2. **防连点家族**（R-3 + 未验证 5 条 + Y-38/39）一个共用「在途禁用」组件一次修完。
3. **Y-24 令牌守卫家族** 在 APIClient 统一入口加一道，25+ 处一次收。
4. 86 条未验证里 22 条重大：额度恢复后续跑验证（缓存在），或修之前逐条人工核实。
...
- [x] **M-1 首页活动卡生产版显假数据** — ✅ 2026-06-11 修（commit `fc57edb`）：LifeTab 加 `loadedEvents`/`loadedBusRoutes` 两个 @State，`.task` 生产版拉 `EventsAPI.listEvents` + `BusAPI.listRoutes`（顺手把同卡片群里同样直读 SEED 的巴士卡一起接真），eventsCard/upcomingBus 双轨 `#if DEMO` 守卫，拉失败显「0 件」/「予定なし」不退回假数据。双 scheme BUILD SUCCEEDED。〔遗留同族：packageCard 仍读 SEED.packages，未在本次范围〕
...
> （下段从原 §🔧 下次会话接续清单（2026-05-21）移入 — 鉴权/防代刷/明文密码类，上线前必须处理）
...
### ✅ 已完成 9 条（无人值守 GOAL + 2026-06-05 会话；真值详见 `05_logs/杭田需求_无人值守施工清单.md`）

- [x] ~~**1. 出寮者一览确认页**~~ ✅ 后端 `GET /applications/active`（只读、R4 寮边界）+ 网页只读页（1,2/4 寮分开 + 打印）。commit `9323431`/`0503ef4`
- [x] ~~**3. 改晚自习参加名单**~~ ✅ study roster 增删 + 软删复活 + 并发 race 修复。`dced81c`/`f08ca45`
- [x] ~~**4. 老师当天补录出寮届**~~ ✅ 后端 `POST /applications/by-teacher`（`8ac99c8`）+ **网页 `ProxyApplicationPage` 表单**（2026-06-05 完成，含搜学生接口 + 三路审查修复，`e77dace`/`f3a846e`/`40de4e3`）
- [x] ~~**5. 事案点姓名跳转档案**~~ ✅ `involved_students` + 网页姓名可点跳档案。`fb6bd8a`
- [x] ~~**6. 审批结果通知学生**~~ ✅ 审批终态发邮件（带评论）。`624b7a2`
- [x] ~~**8. 食堂食数导出按钮**~~ ✅ 网页加导出按钮。`61f2639`。⚠️ Google 表格 vs Excel 格式仍待杭田对齐（见下决策段）
- [x] ~~**9. 行事予定接真后端**~~ ✅ iOS `GET /events` 接真后端。`cb38864`
- [x] ~~**10. 老师写评论给学生**~~ ✅ 审批评论输入框。`993a295`
- [x] ~~**12. 朝 / 夜点呼履歴分开**~~ ✅ `session_type` 分朝/夜两份。`9e7b64c`

### 🔴 剩 4 条未做

- [ ] **2. 学生 增 / 删（退寮）**（五-1）— ⏸ **待 itsuki 决策两件**：① 删除（退寮）用什么状态值（`Student.status` 现无 `withdrawn`，是用 paused / transferred / 还是新加值改表）② 增加学生的建账户安全流程（建的学生怎么拿密码 / 首次登录走注册码还是临时密码 / 谁有权建）。两半都要拍板，不擅自发明。
- [ ] **7. 安卓端接后端** — 大工程，整个网络层从零写。另见 §🐞「Android 整个无网络层」。
- [ ] **11. 点呼结束后改判**（B1）— ⏸ **待 itsuki 决策**：撞规格 §11 故意冻结（结束返 409）。杭田要事后随时改，跟规格冲突，要么改规格要么维持。
- [ ] **13. 网页 5 处假「已发推送」文案** — `index.html` 5 处写「已发推送」实际 `push.py` 是死代码。杭田明说不要推送，要删 / 改成真实状态。清理类、未做。
...
> （下段从原 §🛠️ T 移入 — itsuki 5-28 拍板「都进 v1.0」的宿舍实物申請表实装，剩 Android / 老师网页 / 文档同步）
...
## 🔴 demo scaffold（演示用临时代码）清理 — v1.0 产品化前必做

> （从原 §🔴 高优先级段首条移入，权威备忘 memory `project_demo_scaffolds_to_remove_before_v1.md`）
...
> （下面 2 条从 §N+ progress_overview 退役提取段移入）
- [ ] **Android 签名密钥（keystore）备份方案** — 原阶段 8 部署清单条目：keystore 本地 Mac + 后端服务器加密 + 纸质密码三份备份。Google Play 上架用的签名密钥丢了 = 这个应用永远没法更新，上架前必须落实
- [ ] **扣分自动月结算（cron 定时任务）是否已实装待确认** — 原阶段 3 条目：月累计 4.0 分罚扫 / 8.0 分禁足的「自动月度结算」。后端已有扣分查询接口（6-02 IX-008b），但「每月自动结算触发处分」的定时任务有没有做过要查 `discipline.py` 确认，没做就是 v1.0 缺口
> （下条从原 §📋 旧审查 backlog A.5「v1.0 上线前必做」移入 — GitHub 公开仓库含学生真名）
...
> 现状：老师→学生目前只有「公告（お知らせ）」= 一对多广播、全员可见。本功能是「**一对一私信**」= 老师单独对某个学生发、仅该生可见 —— 是**新的通信通道**，不是公告的扩展。

- [ ] **5 端联动大功能，实装前走 new-feature skill**。粗范围：① backend 新表（私信会话 + 消息 + 老师↔学生指向）+ 新接口（发送 / 拉取 / 已读）② teacher_web 老师侧「选学生 → 发消息」界面 ③ iOS 学生侧「收件箱 / 会话」界面 ④ Android 同步 ⑤ spec + system_features 记一笔。
- [ ] **实装前要 itsuki 先拍板的设计点**（别 CC 自己定）：㋐ 学生能不能回复（双向对话）还是只能老师单发？㋑ 要不要 push 通知？㋒ 进现有「申し込み」信箱图标 tab 还是新开入口？㋓ 老师能发给「任意学生」还是「仅自己管辖的学生」（跟老师权限分级系统联动，见 §B 权限分级段）？
- [ ] **版本归属待定** — itsuki 还没定 v1.0 还是 v1.1+，排期时再定。
...
- [ ] **AC 叙事 v0.22.x CC 起草版待 itsuki 审改**（iCloud 共用池 `2_我挑的/2026-06-11_v0.22.3_版本史插空细分_AC叙事_CC起草.md`，🤖 标记，审改后才算定稿）
- [ ] push 前内容盘点 + 推 main 与 143 个标签 — 走 `00_admin/handoff/push准备_交接提示词_2026-06-11.md`（itsuki 要排除部分文件不公开；push 动作逐步等 itsuki 明示）

## ⚡ 2026-06-11 页面/功能响应时间优化（性能加固 — v1.1，itsuki 拍板）

> itsuki 2026-06-11 拍板：「某个页面或者功能的响应时间也要缩短，做优化。这个是另外一个大工程，放 v1.1 冻结清单。**v1.0 的目标是可用**。」

- [ ] **iOS（及各端）页面/功能响应时间优化** — v1.0 只求「能用」，不为性能停工。v1.1 再做：① 先实测找出慢点（页面首屏、列表加载、点呼/申请提交往返等）② 按实测数据定优化项（接口并发/缓存/懒加载/骨架屏等），不凭感觉猜。具体哪些页面待 itsuki 指定或实测暴露。**大工程，整段归 v1.1。**

## 📢 2026-06-11 安卓老师公告同类「孤儿页/无演示数据」隐患待查（v1.0 安卓同步上线）

> 起因：6-11 iOS 发现 `AnnouncementListView` 是孤儿页（代码在、老师网页能发、但 iOS 学生从任何地方进不去），且演示版三 load 无 DEMO 分支报通信错误。已修 iOS。v1.0 = iOS+Android 一起上，安卓很可能同病。

- [ ] **查安卓 `AnnouncementsScreen.kt` / `AnnouncementDetailScreen.kt` 有无主页入口** — 像 iOS 一样确认安卓主页有没有「お知らせ」入口导航到公告列表，没有就补（对标 iOS announcementCard）。
- [ ] **查安卓演示版公告是否有假数据 + 不连真后端** — iOS 演示版补了 6 条真实宿舍公告 + DEMO 分支；安卓演示版若同样直连后端会报错，需对齐（可复用 iOS 那 6 条真公告日语原文）。
- [ ] **安卓是否对齐 iOS 的 3 个 AI 功能**（公告翻译/总结 + AI 头像）— 安卓有自己的路子（ML Kit 翻译 / Gemini Nano 总结），待 itsuki 拍板做不做、v1.0 还是 v1.1。

## 🧰 2026-06-11 管理体系二期遗留（2 条）

- [ ] **文件批注回填** — `00_admin/文件批注.tsv` 已种 42 条，剩 ~1310 个文件未批注（`bin/generate_file_inventory.py` 每次收尾报清单）。分批补：每次收尾补当次新文件 + 顺手补 20-30 个存量，不专开会话
- [ ] **CI 首跑红绿确认** — push 恢复后（等 GitHub 历史清理 + itsuki 明示）看 GitHub Actions 页签：`test.yml`（后端 pytest，5-21 建从没跑过）+ `ci.yml`（老师网页构建，6-11 建）两条流水线首跑结果，红了当场修配置

## 🔍 2026-06-11 5 端命令实测验证发现（harness 评估会话 — 3 条，按拍板只记录不当场修）

- [ ] **backend ruff 9 个 E402**（import 不在文件顶部）— 集中在 `app/routers/discipline.py`：`_JST` 时区常量写在 import 语句前面导致后续 import 全报 E402。风格问题非逻辑 bug，顺手修时把常量挪到 import 块之后即可
- [ ] **iOS `BUILD.md` 演示版构建说明过时** — 还在教手动加 `SWIFT_ACTIVE_COMPILATION_CONDITIONS="DEBUG DEMO"` 的老办法，现成 `TomoshibiAppDemo` scheme 已取代（CLAUDE.md 防坑段已写新版命令）。改 BUILD.md 该段为「用 TomoshibiAppDemo scheme」
- [ ] **后端测试数 373→367 待查一句** — 6-09 记录 pytest 373 全过，6-11 实跑 367 全过（且当天别会话还新增了 4 个 rollcall 测试）。没有红灯，但少的测试是哪次改动删的值得 `git log -- tests/` 查一句确认是有意删除

## 🧰 2026-06-10 优化项目框架会话提取（管理体系类）

- [ ] **联动规则单一真值改造** — file-linkage SKILL.md 的规则矩阵改成由 `sync-rules.sh` 自动生成（写生成脚本）+ 提交时两边条数一致性检查。顺带写入两条 CC 行为约定：新建重要文件时当场判断加联动规则 / 退役文件时清光机制层引用（progress_overview 退役 22 处清理是模板）
- [x] **ac-radar 空输出改状态行** — ✅ 2026-06-11 启动流程大改版做掉：startup_check.py 永远印一行状态（新提醒 N 条 / 今日已提醒过 / 未到条件），实跑验证过
- [x] ~~协作板公告过期清理~~ — 作废：session-coord 2026-06-11 整体停用（itsuki 拍板「不好用也没用」），公告机制随之消失
- [x] ~~启动流程加 handoff 交接件扫描~~ — 2026-06-11 拍板不扫：交接件 itsuki 都当场用，归档责任改由被交接 AI 承担（handoff/README.md 规则 5：交接词模板末尾强制附归档义务段）

## 🔑 老师权限分级系统实装（设计已定方向 / 待落地 — 大活，被清扫删除打断）

> 起因：itsuki 拍板「职位=只是标签 / 权限=按功能逐个开关分组」+ 登录页方案 B。讨论停在「列全 26 个功能 → 分 5 组」。设计细节见 raw `2026-06-10`。

- [ ] 登录页改方案 B（一页全显：先选功能/账号类型再选老师卡片）
- [ ] 9 个头衔（校長/寮務部長/寮務課長/国際交流部長/国際交流課長/管理係/寮監/学習担当/寮務一般教師）做成建账号时可选的标签
- [ ] 5 个权限组（op 全权限非个人 / 寮管理者全权限 / 一般宿管 / 一般宿管+晚自习 / 申請承認専用）+ 每组短介绍
- [ ] op 账号（账号名 op，密码 itsuki 自己存，**绝不写进代码/仓库**，用环境变量）
- [ ] 权限 = 按功能逐个开关（职位只当显示标签）；先列全 26 个功能逐个确认归哪组
- [ ] 食堂名单已存在（食数表 Excel 导出 §7.7），跟需求老师确认要「谁不吃饭」（已有）还是「全员名册」（要补）

### 🔍 2026-06-11 权限分级 codex 审查（gpt-5.5 xhigh 只读审 + CC 独立核实）— 0 阻塞 / 5 重大 / 2 次要 / 1 建议

> 后端+老师网页两端权限分级已实装（4 条验收全过）。codex 复审挑出下列问题，CC 已逐条独立核实裁决。**Finding 2 已当场修，其余均涉产品/设计/安全决策，按规则列清单等 itsuki 拍板。**

- [x] **F2 已修**（2026-06-11）：`student_promote.py /renewal-start` 漏迁——还在用旧 `require_teacher_roles(*_ADMIN_ROLES)`。§6 把 renewal 列为「学生账号管理」簇管理动作 → 已改挂 `require_permission(C_STUDENT_ACCOUNT, MANAGE)`，无测试破坏（buchou 仍 200 / gakushu 仍 403），body 寮过滤保留。
- [x] **F1 已修**（2026-06-12，itsuki 拍板「现在补」）：codex 报 14 端点缺寮过滤「被放大」，CC 核实只有 `admin_accounts` 的 list/password-reset/unlock **3 个**是本次真放大的（改动前 ADMIN_ROLES 3 职位 → 改动后所有 M 组含分寮老师、且无寮过滤）。已修：list 查询加寮过滤、password-reset/unlock 加 `_assert_student_in_dorm`（403 FORBIDDEN_DORM）。**其余 11 个（rollcall 536/625、applications 790、study 573/639/675、incidents 82/134/159/184/246）是改动前就有的既有问题、跟本次无关 → 见下方「F1-遗留」安全加固任务**。
- [ ] **F1-遗留（安全加固·与权限分级无关）11 个端点既有寮边界缺失**：`rollcall` board/summary、`applications` 审批、`study` absence/decide/cancel、`incidents` 全部 — body 没寮过滤，任何老师（改动前后都是）可跨寮查看/写对应数据。改动前裸 `get_current_teacher` 就如此，本次权限分级没放大。设计 §5「寮过滤正交叠加」原则要求补。待拍板：v1.0 前补还是推 v1.1（工作量：11 端点逐个加寮过滤 + 定 404/403 语义）。
- [ ] **🏠 寮过滤整体方向待定（2026-06-13 itsuki 新决定 — 与上条 F1-遗留方向相反，必须一并理清，单独立项）**：职位退出鉴权时 itsuki 提出「所有老师可查看所有学生，仅修改受权限限制」。这跟 F1-遗留「补 11 端点寮过滤」**方向相反**（一个去过滤、一个补过滤），不能各做各的。牵动范围大：① 推翻冻结规格 R4（男寮老师只看男寮[1,2] / 女寮只看女寮[4]）；② 25+ 端点都调 `dorm_units_for_teacher`；③ **男女寮隐私** — 改成「都能看」后男寮男老师将能看到全部女生敏感信息（房间号/出入记录/事案/扣分）；④ 设计文档 §5「寮过滤正交叠加」注要改；⑤ `student_profile`（学生档案查看，仍按 `_GUIDANCE_ROLES` 职位判，本轮没动）随此一并处理。**需专门一会话把「规格/隐私/查看 vs 修改边界/端点」一次理清，不随手改**。itsuki 倾向已明确，但 CC 判断这是推翻冻结规格 + 隐私决定，不在职位退出鉴权那批里草率做掉。
...
- [x] **F6 已做**（2026-06-13，itsuki 拍板「直接全部允许查看」）：前端导航不再按 role 判显隐。`Shell.tsx` 删 3 个职位列表隐藏的菜单（代録/事案記録/教員管理）改无条件显示 + 删死 prop `currentRole`（`App.tsx`）。所有老师可查看所有功能页，增删改限制由后端权限闸按权限组把关。**不做置灰**（§5 矩阵无 ✕、人人至少有查看权，菜单层面无需隐藏/置灰）。commit `3382021`。
...
- [x] **F8 已做**（2026-06-13，itsuki 拍板「职位规则彻底取消」）：删 `applications.py` 代録/proxy-candidates 的 `_DAIROKU_ROLES` 职位二次检查（端点已挂 `require_permission(C_APPROVAL)`）。连同 `dorm_life` 4 审批端点 + `notifications` + `meals`/`teachers` 死代码 + `deps.require_teacher_roles` 闸函数一并清，职位全后端不再参与鉴权。commit `365f3e7`（pytest 371 passed）。

---


## 🔍 2026-06-10 iOS 全量审查 — 次要/小/已推翻（背景与重大条见 §A 同名段）

### 🟡 次要 — 已验证确认（合并同根因后 44 条）

**前后端对齐类（14）**
- [ ] Y-1 APIClient 不解析 FastAPI 默认 422 数组形 detail——所有 Pydantic 校验错（时刻倒挂/字段超长/邮箱非法等）一律退化成笼统「入力エラー」，用户不知道错哪（`APIClient.swift:243-268`；6 个代理各自撞到，影响所有表单）
- [ ] Y-2 注册码有效期文案失配：iOS 对学生显示「5 分钟有效」，后端真值 30 分钟（用户可见文案 `AuthStubs.swift:2053`；注释同病 `NetworkModels.swift:261` / `AuthAPI.swift:34`）
- [ ] Y-3 注册客户端漏邮箱格式校验（连 trim 都没有），无效邮箱拖到后端 EmailStr 422 才报笼统错（`NetworkModels.swift:289-291` + `AuthStubs.swift:1275-1283`）
- [ ] Y-4 房号首位字母不加 M/W 前缀直送，后端只认 M/W → 必 422（`AppStore.swift:38-42`）
- [ ] Y-5 点呼上报正文 iOS 不限字数，后端上限 2000 字，超长 422 不可读（`HomeStubs.swift:2143-2153`）
- [ ] Y-6 通知中心包裹卡把 expired/discarded 两个后端状态映射成「荷物が届いています」（`AppStore.swift:966-976`）
- [ ] Y-7 profile 接口 limit 封顶 100 且无分页，点呼履历只覆盖约 7 周，撑不起注释声称的「12 个月」（`StudentProfileAPI.swift:41-46`）
- [ ] Y-8 学習卡 TODO 注释指向的 /study/today/state、/study/attendance/mine 两端点后端不存在（`AppStore.swift:599,771`）
- [ ] Y-9 推送 payload「type」键契约只写在 iOS 侧，后端 payload 构造与 schemas 均无此约定（`TomoshibiApp.swift:107`）
- [ ] Y-10 杂项申请一览 GET /misc-requests/mine 后端有、iOS 没调——学生提交后在 app 里看不到状态（`misc_requests.py:50-52`）
- [ ] Y-11 指导记录开示请求 POST /students/{id}/guidance/disclosure-request 后端有、iOS 无入口（`guidance.py:175-184`）
- [ ] Y-12 AnnouncementsAPI.deleteReply 有后端有包装、无界面入口——学生删不了自己的回复（`AuthAPI.swift:98-104`）
- [ ] Y-13 老师代录动作 application.submit_by_teacher iOS 没映射，学生履历显示英文原始串（`StayListStubs.swift:1777-1786`）
- [ ] Y-14 EventItem.id 用「日期+标题」拼接、丢弃后端真 UUID，同日同名行事撞 id（`ScheduleStubs.swift:270,388-403` + `SeedModels.swift:134-137`）

**错误处理/三态缺失（9）**
- [ ] Y-15 AppStore 的 loadXxx 系列把 401 吞成普通失败：不清令牌、不跳登录（`AppStore.swift:1001-1091`）
- [ ] Y-16 loadMe 扣分汇总 try? 静默吞错——拉失败时扣分/迟到/缺席数恒显 0 当真值（`AppStore.swift:216-220`）
- [ ] Y-17 通知中心三个数据源拉取全部吞错、无三态（`AppStore.swift:1131-1137,989-991,458-460`）
- [ ] Y-18 荷物受取履歴吞错后显示「なし」（`MyPageStubs.swift:2107-2109`）
- [ ] Y-19 提出済み一覧（在线学习）网络故障显示假空态（`StudyOnlineForm.swift:364-402`）
- [ ] Y-20 公告一覧/详情失败态只有一行字无重试按钮 + 每次进入全屏转圈盖住已有缓存（`HomeStubs.swift:2638-2647,2797-2806`）
- [ ] Y-21 首页点歌/遗失物卡失败显示假空态「0 件」（`HomeStubs.swift:1076-1148`）
- [ ] Y-22 点歌/遗失物提交失败吞掉后端具体错误、401 不清令牌（`CommunityStubs.swift:844,979,1267`）
- [ ] Y-23 列表 .task 被取消时误弹「通信エラー」toast 到下一个页面（`DormLifeForms.swift:250-253`）

**令牌守卫家族（1 条总）**
- [ ] Y-24 「令牌为空不发请求」守卫缺失是全局性问题——已知 #9 只记了 submitOuting 1 处，实际 25+ 处：AppStore 10 处（451,464,473,497,633,676,981,1001,1022,1043）/ StudyAPI 全部 6 调用 / DormLifeForms 6 处 / CommunityStubs 2 处（834-837,1257-1260）/ HomeStubs 三上报（2150-2160）/ ApplyListView.load（`ApplyStubs.swift:116-145`）；另 StayForm.submitAsync 成功路径缺在途切号守卫（`ApplyStubs.swift:1074-1076`）。建议做成统一入口一次修。

**生产分支假数据/死件（12）**
- [ ] Y-25 StayList 生产保留「未登录显 Mock」回退 + 带非 UUID id 进详情/编辑走 mock 假数据假提交（`StayListStubs.swift:767-777,1271-1280,1509-1522`）
- [ ] Y-26 审计履历转换器读 SEED.user.name——loadMe 失败窗口操作人显示假名「リュウ イヒ」（`StayListStubs.swift:1764`）
- [ ] Y-27 行事/巴士 load() 未登录回退 SEED.events / BusListMock，假数据无守卫编进生产二进制（`ScheduleStubs.swift:91-95` + `BusListStubs.swift:276-279`）
- [ ] Y-28 処分履歴页是写死的静态空态，恒显「処分歴はまだありません」（`MyPageStubs.swift:1843`）
- [ ] Y-29 EventDetailView「カレンダーに追加」生产假动作只弹 toast 不写日历（`CommunityStubs.swift:1733-1735`）
- [ ] Y-30 LostNewView 三件套：「画像」标必填但是死控件 / 拾得日時硬编码 2026-04-22 15:00 / 必填红星不校验留空可提交（`CommunityStubs.swift:706-781`）
- [ ] Y-31 通知中心过滤 pill 与生产类型不匹配：公告无法筛、「活動」pill 永远空（`CommunityStubs.swift:98-106`）
- [ ] Y-32 変更履歴只存内存重启即丢，注释承诺「从后端拉」未实装（`MyPageStubs.swift:714-756` + `AppStore.swift:368-376`）
- [ ] Y-33 修繕表单「写真を追加（任意）」纯装饰死控件（`ApplyStubs.swift:1765-1779`）
- [ ] Y-34 ApplyPreviewView 假提交无守卫：写死申请人「Nishimura Aoi」+ 不调后端直接报成功（`ApplyStubs.swift:2030-2118`）
- [ ] Y-35 GenericApplyForm 收集字段提交丢弃：外出「交通手段/連絡先」、来訪者必填「帰寮予定時刻/連絡先」都不发后端，学生填的信息静默丢（`ApplyStubs.swift:1726-1737,1788-1792,1916-1923,1980-1985`）
- [ ] Y-36 SongReportSheet（点歌举报）无守卫编进生产：读 SEED.songs + 假封禁模拟（`CommunityStubs.swift:1781-1798`）

**表单交互（5）**
- [ ] Y-37 注册完成页文案承诺「可用邮箱登录」与实际功能不符（`AuthStubs.swift:1497`）
- [ ] Y-38 StudyAbsenceForm 提出无防连点，第二发 422 弹在完成页上（`ApplyStubs.swift:1510-1547`）
- [ ] Y-39 連絡先・部屋編集「保存する」无在途守卫：连点重复 PATCH 且 router.back() 弹两层（`MyPageStubs.swift:975-990`）；修改届提交无视觉反馈 isSubmitting 不禁用按钮（`StayListStubs.swift:1462-1475`）
- [ ] Y-40 StayForm 必填校验不 trim：纯空格绕过「理由」必填原样发后端（`ApplyStubs.swift:528,542-544,1014`）
- [ ] Y-41 編集页预填只在 onAppear 跑一次，资料未到时把占位「—」填进邮箱/电话不回填（`MyPageStubs.swift:876`）

**时区/历法（3）**
- [ ] Y-42 公告/学习完成时刻格式化没固定 Asia/Tokyo——手机时区非日本的留学生看到的时间偏移（`HomeStubs.swift:2924-2972,1966-1971,2749-2758`）
- [ ] Y-43 combineDateAndTimeISO 用设备时区拼合实施日时（同文件 formatYMD 已修固定 JST、此函数漏改）（`ApplyFormSupport.swift:107-120`）
- [ ] Y-44 Calendar.current 和暦坑两处：系统设和暦时「今月の点呼」统计全归零（`MyPageStubs.swift:35-38`）+ 減点グラフ 12 个月曲线全 0（`MyPageStubs.swift:1656-1665`）

**导航/状态/组件（10）**
- [ ] Y-45 注册各步「戻る」用 router.go() 压栈而非 back()，导航栈无限增长（`AuthStubs.swift:1133/1279/1403/2125`）
- [ ] Y-46 401 踢回登录页时全局 sheet/breadcrumb 浮层不关闭，盖在登录页上且可能残留前用户内容（`RootView.swift:47-51`）
- [ ] Y-47 OutingDetailView 401 只 router.back() 不跳登录，同文件其他 401 处理不一致（`ApplyStubs.swift:2697-2699`）
- [ ] Y-48 showToast 不取消上一个清除任务——连续两条 toast 第二条被提前抹掉（`AppStore.swift:523-531`）
- [ ] Y-49 NFCTagReaderSession 创建失败（init? 返回 nil）时 continuation 永不恢复，签到 Task 永久挂起（`ST25DVWriter.swift:66-72`）
- [ ] Y-50 生产 UI 文案混简体中文：「学号」（`AuthStubs.swift:1727`）/ FeedbackSheet「反馈」、学习卡「請假」「签到」（`HomeStubs.swift:2018,2024,407,482,499`）
- [ ] Y-51 巴士日分组头把当日首条班次 note 当整组标题，其余 note 永不显示（`BusListStubs.swift:198-205,371-377`）
- [ ] Y-52 承认时附带的 comment 被当差戻警告红卡显示（`StayListStubs.swift:720-722,1039-1063`）
- [ ] Y-53 SEED.user 用 nonisolated(unsafe) 全局可变变量承载生产真实用户数据、并发检查显式关闭（`SEED.swift:32`）；User.placeholder 缺省 isStudyTarget/isOverseas=true，拉失败窗口普通学生渲染成「学習対象+留学生」（`SeedModels.swift:24-36`）
- [ ] Y-54 downloadOnlineContract 绕过 APIClient 用 URLSession.shared：无 15 秒超时、base URL 规则手抄第二份（`StudyAPI.swift:79-112`）；GlassSheet onClose 声明从未调用 9 处死回调（`GlassSheet.swift:8`）；ContractFilePicker photoItem 不复位、删附件后重选同照片无反应（`ContractFilePicker.swift:69-73`）；TArea 的 TextEditor 没设 .scrollContentBackground(.hidden) 自定义底色被盖（`Field.swift:80-101`）；前台点推送横幅同条通知重复插入两次（`TomoshibiApp.swift:82-99`）

### ⚪ 小 — 已验证确认（合并后 12 条）

- [ ] X-1 注释漂移合集：`Route.swift:2`「32 case」实际 54 / `StayListStubs.swift:8-11` mock 承认链与 approval_chain.py 漂移 / `ApplyStubs.swift:1576` 外出表单「纯演示桩」注释失实（出租车时刻已真发后端）/ `StudyAPI.swift:4-9` 文件头只列 1/6 端点 / `SeedModels.swift:6` account 注释 4 位实际 6 位 / `GlassCard.swift:4-5` 写「iOS 26 不降级」与 iOS 16 决策矛盾 / `ScheduleStubs.swift:372-377` fetchRange 注释「今年」代码取去年 / `StudyOnlineForm.swift:504-507` 契約書 409 注释分支不可达 / `AppStore.swift:979-987` loadXxx 注释自称带守卫实际没有
- [ ] X-2 死代码合集：MealCheckbox（`ApplyStubs.swift:1377-1399`）/ Array.subscript(safe:)（`StudyOnlineForm.swift:549-553`）/ OnboardingView+PwResetView 无导航入口（`AuthStubs.swift:210-326,1932-1993`，PwReset 还混中文「後台」）/ Ic 图标 13 个零引用含已砍寮ウォール残留（`Ic.swift:16-40`）/ IOSStatusBar 假状态栏（`IOSStatusBar.swift:6-23`）/ SpecialBusRoute.isNext 死字段（`BusListStubs.swift:60-217`）/ ApplyKindMapper study_absence 死条目（`ApplyKindMapper.swift:14`）/ BusListStubs 401「セッション切れ」死文案（`BusListStubs.swift:284-290`）
- [ ] X-3 日语注释违反中文铁律（新文件，非老 Stub 豁免范围）：`APIClient.swift:2-40` 等网络层 3 文件 / `ApplyKindMapper.swift` 全文 / BreadcrumbOverlay + BottomNav
- [ ] X-4 App 内显示版本 AppVersionTag 写死 0.15.0、与 project.yml 双源维护且已落后（`TTokens.swift:103-111`）<!-- VERSION_OK -->
- [ ] X-5 RollCallAPI.checkin 包装的端点要老师令牌、学生调必 403（`RollCallAPI.swift:20-28`；验证代理间有分歧：一票认为「保留给方案 B」前提还成立，待硬件联调时一并定）
- [ ] X-6 点歌卡 artist 空时显示「曲名 · 」尾巴分隔符（`HomeStubs.swift:1083`）
- [ ] X-7 タクシー予約时刻原样显示「HH:MM:SS」带秒（`StayListStubs.swift:74,987-989`）
- [ ] X-8 audit 子请求 catch-all 把 401 当非致命吞、不清令牌（`StayListStubs.swift:786-794`）
- [ ] X-9 详情页「セッション ID」是前端拼的假 ID，后端真 session_id 解码了不用（`MyPageStubs.swift:1277-1281`）
- [ ] X-10 PrimaryButton destructive 分支优先于 disabled，禁用的红按钮仍显示可点外观（`PrimaryButton.swift:28-44`）；PageHeader 打开面包屑没包 withAnimation 硬切（`PageHeader.swift:46-51`）；ToastView 全屏 frame 没关命中测试、显示 2.2 秒内挡下方点击（`Toast.swift:9-19`）
- [ ] X-11 后端有 iOS 没接（信息性）：点呼上报履历 GET /rollcall/reports/mine 只写不读，学生上报后无处回看（`rollcall.py:818-820`）/ 登出 DELETE /sessions/current 不调、只清本地令牌（`auth.py:215-227`）/ 巴士单条详情不调（影响为零）
- [ ] X-12 未验证的小问题残余（额度中断，列标题备查）：后台/杀进程期间到达的推送不点横幅就进不了通知中心 + 重启清零（`TomoshibiApp.swift:80-99`）/ 三投稿表单无长度上限校验撞后端 max_length（`schemas.py:1150-1205`）/ breadcrumb 跳级按值匹配、栈里有重复路由时跳错层（`BreadcrumbOverlay.swift:58-70`）

### ❌ 对抗验证推翻的 8 条（列出防以后重复报）

点呼详情查不到记录回退「時間内 朝点呼」/ RouterStore.go 无防重复推入 / ApplyKindMapper 日语注释（与 X-3 重复报的变体被判重复）/ 公告页无 #if DEMO 演示必报错 / loadCleaningHistory 缺守卫 / RollCallAPI.checkin 保留前提不成立（另一票判成立，见 X-5）/ 杂项 withdraw iOS 无入口 / GET /teachers/public iOS 没调


## 🔍 2026-06-09 深度审查 — 次要/小（背景与重大 2 条见 §A 同名段）
...
- 演示老师 demo 登录 → 学生账号管理只显示 3 演示学生（デモ一郎/二郎/花子），「総アカウント 3 サーバーから取得」
...
- [x] ~~project-overview §0.1 体量表 iOS 文件数 +7~~ ✅ 2026-06-11 完成（commit `cc29127` 全目录对账 1399→1450，iOS 78→84 + §5.6 列出 5 endpoint + ST25DVWriter）
...
- [x] ~~🟡 慶應倒计时日期~~ ✅ 2026-06-08 itsuki 发来慶應SFC 2026 夏秋AO 募集要項 PDF（6-08 公表）→ 提取真日程换掉占位：出願 8/3–9/1（締切9/1）/ 1次合格 10/8 / 2次面接 総政10/17-18·環境10/24-25 / 最終合格 11/2 / 入学 27年4月或9月。首页加 4 张慶應卡 + 大学入試项目卡刷新。已部署
- [ ] 🟠 **筑波倒计时日期疑似占位待核实** — 首页筑波「試験+面接 10-17」「最終合格 11-02」跟慶應一模一样；筑波 R9 募集要項 6-15 才公表，现在拿不到筑波真日期 → 这俩很可能是早先拿慶應日期填的占位。itsuki：6-15 筑波要項出来后核对，是占位就改成真日期 / 或标待公布
- [x] ~~慶應SFC 学部选择~~ ✅ 2026-06-08 itsuki 拍板报 **環境情報学部**；首页面接卡收敛到 10/24-25 + 项目卡/阻塞项同步部署
...
## 🚧 老师网页 Vite 迁移 + 邮件 Resend（2026-06-05 itsuki 拍板）

- [ ] **老师网页 HTML 单文件 → React + TypeScript + Vite**（大工程，未开工）— itsuki 拍板一步到位上 TypeScript、**界面 100% 冻结逐页搬**。完整施工清单 `03_dev/teacher_web/Vite迁移_施工清单.md`（5 阶段 + 铁律 + 现状 16 页面）+ GOAL 提示词 `00_admin/老师网页Vite迁移_GOAL提示词.md`（compact 后粘贴触发）。吸取 5-26「不是我的 web」失败教训。
- [ ] **邮件 SendGrid → Resend** — itsuki 定 Resend（永久免费 3000 封/月）。**待 itsuki 去 resend.com 注册拿密钥 + 绑域名** → CC 改后端 `email.py` 成 Resend 版（带 pytest，没密钥时 dev 模式不真发）。独立小任务、可跟前端迁移并行。
...
## 🧹 2026-06-04 遗失物 + 匿名建議删除 收尾 backlog

- [ ] **teacher_web 单文件副本残留** — `03_dev/teacher_web/v1/Tomoshibi_v3_single.html`（未纳入 Git 的手动导出快照）里还留着旧的「匿名建議 / 食堂 / ロビー」，源码 `src/index.html` 已清干净。要么用源码重新导出覆盖、要么删这个副本。
- [ ] **Android 完整编译验证待补** — 本会话遗失物 + 匿名建議删除 + AppStore 改动，iOS 已 xcodebuild 真编译过、后端 pytest 223 过；但 Android 因本机没装 SDK 只做了静态验证（codex 把关 Kotlin + 结构配平）。有 Android Studio / SDK 环境时跑一次 `./gradlew compileDebugKotlin` 确认。

### 🚌 巴士时刻表按钮 + 接真后端 codex 审查遗留（2026-06-04，另一会话做的，已 commit）

> 本会话做了「申请表单查特別運行便时刻表按钮 + 时刻表页 BusListView 接真后端 + 演示登录凭据改 060217/12345678」，经 codex(gpt-5.5 xhigh) 3 轮审查收敛。以下 3 条是 codex 提出、判断**推迟/不改**的遗留，记此防丢：

- [ ] **`.busList` 底部导航高亮归属** — 从申请页点时刻表按钮跳 `BusListView` 后，底部栏高亮的是「マイページ」不是「申し込み」（`.busList` 在 `Route.swift` 归 `isMyBranch`，首页入口也一样，是既有分类非本次引入）。低优先。codex 给的低风险修法：加一个 `.applyBusList` 路由别名（同一个 `BusListView`、归 `isApplyBranch`），申请入口走别名。涉及 Route / RootView / ApplyStubs。
...
- [x] ~~秋月下单~~ ✅ 2026-06-05（台风 6 号过后恢复营业当天下）— Pi 3A+ 主板 + 5V3A 电源 + 4 色 LED + 220Ω + **10kΩ（ST25DV GPO 上拉，125103）** + 杜邦线 + 5V 风扇 + **万用表 MAS830L（112542）**。
...
> 三个月没动 / 不确定还要不要做 / 可能已被后来的实装取代。**一条没删** — 只归到这层；清理前逐条核实再动。
...
> （注：本段「主会话保留 6 条」已移 §A、「Bot 3 待决策 5 条」+「5-22 五大决策」已移 §D）
...
- [ ] **从 `~/.claude/我的环境.md` 重新生成 `~/.claude/我的环境.html`** — `.html` 停在 2026-06-06，`.md` 已领先一大截（5-11 起就欠：cc-comm-rules + 多 hook；6-06 second-brain-save/web-design-styles；6-10/6-11 anti-ai-flavor 6→8 类 + DMSD skills 8→9 codex-review + 删退役 progress_overview）
  - `.html` 是 `session-start-env-diff.sh` 挂钩对账的版本，落后会让每次启动报 diff
  - 做法：从已是真值的 `.md` 整体重生成 `.html`（itsuki 拍板要不要做 + 是否值得，因为 .md 已经是真值、.html 只是美化版）
  - 验证：重生成后下次 CC 启动 `session-start-env-diff.sh` 不再报 diff
...
- [x] ~~**`progress_overview.md` 4-17 后大改**~~ ✅ 2026-06-10 以退役方式解决 — itsuki 拍板归档（进度叙事由 CHANGELOG 全版本一览 + PROJECT_GUIDE §8.5 时间线取代），文件移 `99_archive/2026-06-10_progress_overview退役.md`，活的待办已提取到下方「progress_overview 退役提取」段

### N+. progress_overview 退役提取（2026-06-10 加 — 退役前从它的待办段捞出的仍可能活着的条目）

> （注：本段原第 1、2 条「Android 签名密钥备份」「扣分月结算 cron 确认」已移 §A）
- [ ] **4 月老 AC 待办是否还要做（itsuki 拍板砍留）** — 原「当前待办」段残留：① 填 4-10 dev_log 的 4 个【】占位符 ② reflection_2026-04-10 的 5 个占位符（已迁 iCloud）③ 项目起源_真实观察.md 的 AC 起源问题（已迁 iCloud）④ 和真人讨论项目（interview_log）。都是 4 月体系的产物，AC 素材体系 6-04 重构过，可能已无意义 — 砍留你定
...
> （注：§R 点呼机块 + §S 日语中文化已移 §B，§T 实物表已移 §A）
...
> （注：本段「demo-only 清理」已移 §A、「补点呼机契约 spec」已移 §B）
...
## 📦 raw（AC 素材正本）迁不迁 iCloud — ✅ 已拍板执行（2026-06-11）

- itsuki 拍板「raw 不放 DMSD 里面，放 iCloud 里面」→ 当天执行：107 个文件迁 iCloud 素材池 `00_原始素材池_两校共用/1_自动产出/raw详细叙事/`（逐字节校验一致后 git rm，commit `d54f16b`），仓库 `05_logs/raw/` 只剩指路牌 README + 4 个别会话滞留文件（各自收尾后由收尾流程搬走）。
- 规则同步：session-wrap §3.1 / §5.5.1.B、ac-radar 双写段、收尾提醒 hook 第 1 项，全改写 iCloud 路径。

## 🗑️ 2026-06-10 清扫/罚扫全 5 端删除（已做完）+ 衍生待办

> 清扫删除本身已完成 + codex 2 轮收敛 + 提交 `e15ae95`（不含别会话 6 个 Android 文件）。下列是衍生待办。

**待 itsuki 拍的（非 bug，CC 替定了默认值等复核）**
- [ ] iOS 首页没点呼时的状态 pill：CC 把「来月より清掃対象」替成「点呼開始前」（中性占位）— 想换措辞告诉 CC
- [ ] iOS/Android 减点明细卡 4–7.9 分徽章：CC 把「罰掃 注意」替成「注意」— 想换告诉 CC
- [ ] Android 假数据 `MockData.kt` 还留「掃除当番交代依頼」（学生换打扫值日的杂项申请示例）— 这是现实打扫值日、不是删掉的清扫审查功能，CC 留着了；想把界面所有「掃除」字眼抹掉就说
- [ ] 老师网页扣分排名小不一致（codex suggestion 级）：3–3.9 分学生在「警告リスト」名单里但那行总分不标橙色（橙色现从 4 分起）— 逻辑没错，要不要统一窗口

**待补的文档联动（system_features + decision_log 已记，per-end 设计日志没补）**
- [ ] BACKEND / IOS / ANDROID / WEB_DESIGN_LOG 各加一条「清扫删除」记录


> （下块从 §🔍 2026-06-10 审查段移入 — 改文档还是改代码待拍板，另见 `00_admin/项目心智模型.md` §6）

### 📄 文档契约修正（CC 已亲自 grep 核实，非代理转述）

- [ ] **{ok,data} 响应包络实际不存在** — 后端 29 个路由全部直接返回数据模型（全局 grep 仅 main.py 健康检查返回 {"status":"ok"}），iOS 也直接解码，**两侧代码一致没毛病**；但 `00_admin/项目心智模型.md` §3 第 1 条 + `01_specs/API_CONVENTIONS.md` 仍把包络写成契约 → 改文档对齐现实（真要包络则两侧大改，不建议）。
...
- [x] **时区根治**：后端写入统一存世界时、读出统一带 +09:00 日本时间（`database.py` 新增 `TZDateTime` 类型，`models.py` 88 处替换）。推翻通宵的 iOS 端「猜世界时」治标修法。
- [x] **iOS 包裹一览页接真后端 + 假数据 demo 守卫**：`PackagesView`/`PackageDetailView`/`MyPackagesView` 改成 `#if DEMO` 用 SEED、生产用 `app.packages`（`GET /front-desk/mine`）；新增共用 `PackageDisplay`；路由 id `Int`→`String`。
- [x] **老师网页宅配登记必须选收件学生**：宅配 Modal 加学生搜索选择器（必选），配后端 `FrontDeskItemCreateIn` 强制 student_id。
- [x] **前台列表按男/女寮过滤**：`GET /front-desk` 用 `dorm_units_for_teacher` 按男寮[1,2]/女寮[4]过滤（itsuki 拍板只分男女寮不按楼）。
- [x] **外出接口过时注释清理**：`OutingsAPI.swift` 时区注释更新。
- [x] **寮監挑收件学生 403（codex 没提、CC 自查逮到）**：账号管理 `/students` 角色集不含寮監→新增 `GET /front-desk/students`（含寮監权限 + 男女寮过滤 + 最小字段）。

### 仍需你决策 / 低优先（codex 次要项 + 数据模型决策）

- [ ] **(需你定) 包裹要不要加「配送業者 / 追跡番号」后端字段**：现在生产版包裹详情只显示后端真有的字段（说明 / 到达 / 状态 / 保管位置），「配送業者 / 追跡番号」仅 demo 门面有。要全字段对齐就给 `FrontDeskItem` 加这俩列（要 Alembic 迁移 + 老师登记表单加输入）。你说一声我再补。
- [ ] **(次要) 老师网页宅配提交的 `body as unknown as FrontDeskCreateIn` 强制类型转换**（`FrontDeskPage.tsx`）：可改成可辨识联合类型让 delivery 分支 student_id 必填，纯类型优化、运行时已由「必选才能提交」保证（codex 第 1 轮 minor）。
- [ ] **(次要) `loadMyPackages` 拉取失败与「真没包裹」界面无法区分**（`AppStore.swift`）：失败已保留上次数据（不清空），仅从未成功过时显示「なし」同空态。要严谨可加加载/错误态（codex 第 1 轮 minor，v1.0 可接受）。
- [ ] **(次要/防御) front-desk 过期逻辑**：`expired`/`discarded` 两个状态 v1.0 无任何代码会置成（只有 DB 约束/类型列了）。前端已防御性处理显示，但「包裹到期自动标 expired」的后端逻辑本身没实装，将来要做。
...
> （下块从原 §🛰️ 点呼机第 5 端段移入，实装顺序见 §B 同名段）

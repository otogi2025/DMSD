...
- [ ] 🔴 **项目文件总览升级为"项目唯一入口"**（2026-05-04 itsuki 拍板）
  ...
  - 升级后必含 4 类信息：
    ...
    3. **联动规则**（**新增** — 改了 A 必查 B / 改了 A 必改 B；从 CLAUDE.md「文件连锁结构」段 + `00_admin/hooks/lib/sync-rules.sh` 13 条规则 + `00_admin/文档同步点清单.md` 整合搬过来）
    4. **状态 / AC 价值**（已有）
  - 同步机制：每次新建/删除/改/移动文件 → 项目文件总览 必须当场更新（已经在 CLAUDE.md「文件连锁结构」第 7 条写了，要强化执行）
  - 替代后果：
    - `CLAUDE.md「文件连锁结构」段` 改为指针 → 项目文件总览
    - `00_admin/文档同步点清单.md` 部分内容可挪到项目文件总览
    - `00_admin/hooks/lib/sync-rules.sh` 保持代码化（人类可读版去总览）
  - **不做成 skill 的理由**：联动规则需要永远在线（每次改任何文件都要查），不是触发型；已有 hook + sync-rules.sh 自动检查，加项目文件总览作为人类可读源即可
  - 工作量预估：中（~3-5 小时）— 要遍历所有文件 + 写联动规则 + 整合 3 个分散源

...
## 🐞 2026-06-06 通宵 codex 审查留项（4 轮对抗复审，绝大多数已当场修+提交，下面是需你决策的）

> 起因：通宵审几个并行会话的混乱改动，派 codex（gpt-5.5 xhigh）4 轮复审到收敛。能当场修的（10 条）都修+验证+提交了；这几条要么是跨端功能、要么要你定数据模型，故留。详见 raw `2026-06-05` 末尾「通宵 codex 审查」段。

- [ ] **iOS 包裹「一览页」没接真后端**（`Features/Community/CommunityStubs.swift:206/300`）：`PackagesView` + `PackageDetailView` 仍无条件读 `SEED.packages`（静态假数据、无 `#if DEMO` 守卫 → 生产也漏假包裹）。这几个会话只接了「通知中心『宅配』标签」（`AppStore.packages` ← `GET /api/v1/front-desk/mine` 真数据），这个专属页没接。完整接它要：① Route 的 `homePackageDetail(id:)` id 从 `Int` 改 `UUID`（连带 RootView）② 页面显示「配送業者 / 追跡番号 / 保管場所」后端 `FrontDeskItem` 根本没有——**要你定包裹要不要存这些字段** ③ 状态 `待領/領済` ↔ 后端 `pending/notified/picked_up` 映射。属「v1.0 上线前必接的 scaffold」，跨端 + 需求决策。
- [ ] **老师网页：宅配登记界面要求选收件学生**（配套今晚后端新加的校验）：后端 `FrontDeskItemCreateIn` 已强制 `kind=delivery` 时 `student_id` 必填（不填学生端永远收不到）。老师网页登记表单要对应做成「宅配必须选学生」，否则老师会撞 422。
- [ ] **(次要) `GET /api/v1/front-desk` 是否按担当寮过滤**（`front_desk.py:54`）：任意老师令牌现可列全量前台记录，要不要按老师管辖寮收窄，待你定（codex 提的次要项）。
- [ ] **(次要) `OutingsAPI.swift` 一处过时注释**：datetime 保 String 的注释，在 iOS 解码器统一加无时区兜底后已可清理（不影响功能）。

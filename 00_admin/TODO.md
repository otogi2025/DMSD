...
- [ ] **ac-radar 空输出改状态行** — startup_check.py 没提醒时也印一行「今日已提醒过 / 无到期项」，否则空输出无法区分「没提醒 / 已提醒过 / 坏了」（6-10 启动时误判过一次）
- [ ] **协作板公告过期清理** — 公告超 24 小时自动清或标过期（现在只增不减）
- [ ] **启动流程加 handoff 交接件扫描** — dmsd-startup 加一步：扫 `00_admin/handoff/` 报告活着的交接件（现在 6-05 的版本号收尾交接件躺了 5 天没人接）

...
> **背景**：itsuki 找 Gemini 学点呼机原理时发现旧方案矛盾 → 拍板架构反转（手机写 ST25DV、树莓派被动收）。`hardware_design.md §2.3` + decision_log 已改。反转牵出防代刷断链 + 连带文件 + 后端选址，列待办：

- [ ] **🔴 防代刷机制重新设计（高优先 — 阻塞后端 A-010）** — 旧「nonce 从贴纸传手机 + ECDSA 签名」链断了。三缺口要 itsuki 定方案：① 手机只写明文 ID 会被伪造 → 仍需手机签名 ② 防重放 nonce 谁来发（点呼机收到后向后端要？手机签名带时间戳 + 后端校时间窗？）③ 挡不住借手机/借卡到现场代刷 → 靠 v1.0「播报 + 老师看脸」兜 + v2.0 人脸识别。**定方案前，后端 NFC 防代刷（§🌐 / A-010 / §🚧 A-028）所有 nonce+ECDSA 实装任务挂起重评**

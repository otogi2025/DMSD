# DMSD 全文件审查（2026-05-01）

> **作用**：repo 里**每个 git-tracked 文件**逐一读 + 判断"还在用没"+ "有什么用"+ "下一步建议"
> **范围**：606 个 git-tracked 文件（venv / __pycache__ / DerivedData 等本来就不进 git，已自动排除）
> **方法**：主会话切 7 组 → 7 个 Explore agent 并行扫描 → 合成
> **创建者**：[Mac-主会话调配]
> **和现有 `文件结构指南.md` 关系**：本文是**当前快照 + 价值评估**（动态），那份是**清单 + 反向索引**（静态）。本文发现的过时点（v0.6/v0.7 AC 叙事文件、新增 raw 日志等）已经回填指南。
...
### 1.2 00_admin/AI 协作 + 项目治理（8 文件）

| 文件 | 作用 | 状态 |
|---|---|---|
| `CLAUDE_CODE_记录指南.md` | CC dump 格式 / 详细度 / 当日模板 | ✅ |
| `文件结构指南.md` | 所有文件清单 + 反向索引 | ✅（轻微滞后，本文已对照补） |
| `文档同步点清单.md` | 单源真值表 / 5 AC 核心问题 / 分阶段策略 | ✅ |
| `版本管理SOP.md` | 5 步 bump + 联动 6 处 + commit 前缀 | ✅（v0.7.0 close 首次全套跑通） |
...
### 1.4 00_admin/AC 叙事（11 文件）

| 文件 | 状态 | 价值 |
|---|---|---|
| `v0.3.0_AC叙事.md` ~ `v0.7.0_AC叙事.md`（6 个） | ✅ | 完整 6 版本素材链 |
| `面试准备_索引.md` | ✅ | 6 大类 42+ 题清单 |
| `原创设计_语音播报防作弊.md` | ✅ | ⭐ AC 最强素材之一 |
| `AC_志望動機_素材.md` | ⚠️ | 框架完整 / 内容 0/8 留白等 itsuki |
| `AC_提交_checklist.md` | ⚠️ | 5-10 月 6 个 gate，5 月底 gate 临近 |

...
- 下单：✅ Amazon + ✅ Switch Science / ⏳ **秋月待下单**（台风 6 号通販センター临时休业）。8 张订单截图 + 接法全记进采购清单 HTML。
- AC：模式 2（假设崩）×2 + 6（A/B 取舍）+ 5（认知改变）顶级密度。raw `2026-06-04.md` 22:00-23:00。剩 `led.py` 反逻辑代码（未实装）+ 秋月台风后下单。

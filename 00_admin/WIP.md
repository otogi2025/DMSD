...
> ⚠️ **2026-05-28 收尾时**：本段已严重超 5 条上限（9+ 条）— 砍到 5 条留 itsuki 拍板。建议砍最老的（5-26 晚段-4 / 5-27 凌晨~早段 / 5-27 早段-2 / 5-27 早段-3）
...
- **决策**（4 个层面）:
  1. **架构修正** — web 共用密码 demo 简化方案违反 §3.4 4-30 已拍板的「每老师独立账号密码 R3」→ 实质修漏洞，不是新功能
  2. **设计 ownership** — itsuki 拍板「老师 vs 学生是 2 个独立设计空间，不要对齐 iOS 流程」（模式 5 顶级）
  3. **安全 vs UX 取舍** — CC 主动选「GET /teachers/public 只返 UUID 不返 login_id」防爬虫枚举攻击（codex 后期肯定）
  4. **多 AI 交叉审查方法论** — itsuki 拍板 codex 5.5 xhigh 二次审查发现 3 🔴 阻塞 bug（CC 自检漏的 prior bug + 越权 + lockout 风险）
- **改动**:
  - backend `auth.py` POST /sessions/teacher 加 teacher_id (UUID) 形式 + 修 timedelta import / `teachers.py` 加 GET /public（无认证）+ POST + DELETE + 新增 `TEACHER_ADMIN_ROLES`（不含学習担当）+ LAST_ADMIN 拦截 + IntegrityError catch / `schemas.py` 3 schema / `config.py` CORS 加 8787
  - frontend `index.html` LoginScreen 完整重写 2 屏合一 + 新建 TeachersAdminPage + Shell nav 按 currentRole 过滤 + 砍 select-teacher 中间页 + 砍 anon tab + 3 假数据 + isAnon 逻辑 + 锁定时间显示「30 秒」→「30 分」+ assigned_dorm map 修（4=女寮 而非 2）/ `client.js` 加 3 helper + teacherLogin 改 body 形式 + API_BASE 改绝对 URL
  - 5 设计档案同步：`system_features §3.4` + `BACKEND_DESIGN_LOG §5.1.2b` + `WEB_DESIGN_LOG §5` + `DESIGN_BRIEF` + `propose §4`
- **AC 价值**：模式 1+2 顶级（codex 审查 = 假设崩了→继续→真因）+ 模式 5 顶级 × 2（双服务器分离学到 + 「老师 ≠ 学生」纠正）+ 模式 6 顶级 × 2（共用→实名取舍 + login_id vs UUID 安全设计）→ raw `2026-05-27_老师实名账户登录.md` 7 节详细叙事 + 筑波 inbox `ac_scratchpad_2026-05-27.md` 信号 8 ⭐⭐⭐⭐⭐
- **commit 落地**：`b9f237c` backend + `b444aad` frontend + `1904b18` 设计档案 + `aba0659` codex 修
- **残留**：4 项 itsuki 决策 / 大工程进 TODO §🚀-G（暴露 last_login_at trade-off / DELETE 改 soft delete / EmailStr + login_id 格式校验 / client.js error 包装统一化）

...
**最后更新**: 2026-05-02 by [Mac-主会话] — **iOS↔backend フィールド対齐 F1-F5/Q1/F7 完成**（commit `4be8121`）：(1) **backend** — `Application.reason` 列追加 (F5) / `meals_skip_from/to` 削除 + `meals_skip JSON[{date,meal}]` 追加 (F3) / status CHECK に `returned` 追加 (Q1) / Alembic migration `b2c3d4e5f6a7` 実行済 DB 更新確認 / `meals.py` を range 判定 → entry 直接 lookup に書き換え。(2) **iOS** — `APIClient` / `APIError` 新規作成 (F7 基盤) / `ApplyKindMapper` 新規作成 stay↔外泊 等翻訳 (F1) / `StayForm.submit()` から `student_id` 削除 (F4) / `stay_locations` を `[{kind,name}]` オブジェクト配列に修正 (F2) / `meals_skip` を `expandMealsSkip` で `[{date,meal}]` リストに展開 (F3) / `AppStore.authToken` 追加 (APIClient 同期) / `ApplicationStatus.cancelled` → `withdrawn` 統一 (Q1)。**残り**: F6 backend 4-step 登録 endpoint (~200 行、別会話) / iOS 実 API 接続 wire-up (F6 完了後)。<!-- VERSION_OK -->

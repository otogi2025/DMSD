...
**最后更新**: 2026-05-02 by [Mac-主会话] — **CLAUDE.md §对话规则 §7 加强**：itsuki 提问「claude.md 里有没有提到我需要 claude 辅助我学习」→ CC 盘点发现既存规则只覆盖"概念扫盲"不覆盖"做法诊断" → 7 问澄清需求 → 落地三处：(A) `CLAUDE.md §对话规则` §2 微调 + §7 新建（覆盖技术+AC材料组织+学习方法+自我管理 / 话术模板「你现在这样做是 X，业界标准是 Y，原因是 Z」/ 强度 B-C / 禁用"良好实践"抽象话术 / 必须给具体原因 / 已决定/覆水难收的也要提）(B) 新建 memory `feedback_proactive_diagnose_unknown_unknowns.md` 详细版（覆盖 4 类 / 强度分档 / 正反例 ❌✅ / 反模式 5 条）(C) `MEMORY.md` ⚡ FOUNDATIONAL RULE 块加索引（紧贴 `feedback_be_a_coach_not_executor.md` 下面）。**核心金句 ⭐**「提醒我学习扩展我的思维这个很重要」→ raw/2026-05-02.md §4 AC 候选（自我推荐書「AI と協働姿勢」+ 面试问题#4「学到了什么」均可用）。**dump**: `05_logs/raw/2026-05-02.md` 4 节。**未动**: 任何代码 / spec / 设计文档（`02_design/` / `03_dev/` / `01_specs/` 全部不碰）。 <!-- VERSION_OK -->

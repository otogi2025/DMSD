...
### 2026-06-04 点呼机硬件下单总检查 + LED 驱动改 5V + 真 Codex 审 by [Opus 4.8 1M]

- 起因：itsuki 当天要给点呼机下单约 2 万日元全套硬件，逐件核对截图 + 要「上网搜 + 派 Codex 审，零风险」。
- CC 主动逮漏件（ST25DV 在第三家店没截图 / 烙铁焊锡漏 / microSD）。**LED 致命问题**：审查代理说绿灯 Vf 2.1V「能亮」→ CC 查秋月官方页 g112117 坐实 3.1V（代理错）→ 绿/蓝/白用 3.3V 针 +220Ω 几乎不亮。
- **LED 驱动改 A 方案**（itsuki 拍板）：从 3.3V GPIO 直驱改 **5V 驱动 / 低电平点亮**（信号针当 sink），零成本；否决 B（加三极管，新手多焊点易虚焊）。联动改 5 文件：hardware_design §2.4.1 + ROLLCALL §2 + 接线说明 §4 + decision_log + 采购清单 HTML。
- **派真 Codex**（`codex exec` gpt-5.5 高推理只读）：确认 A 正确可下单 + 抓到 CC「信号针 1.9V」解释错（是电阻压降，点亮针脚约 0V）→ CC 修；加 Codex 2 建议（ST25DV GPO 接 10kΩ 外部上拉 / PN532 接 3.3V）→ itsuki 加买 10kΩ（秋月 125103）+ 万用表（MAS830L）。
- 下单：✅ Amazon + ✅ Switch Science（均 6-04）+ ✅ **秋月**（6-05 台风过后）—— 三家全部下单完成。8 张订单截图 + 接法全记进采购清单 HTML。
- AC：模式 2（假设崩）×2 + 6（A/B 取舍）+ 5（认知改变）顶级密度。raw `2026-06-04.md` 22:00-23:00。剩 `led.py` 反逻辑代码（未实装）+ 等硬件到货装机。

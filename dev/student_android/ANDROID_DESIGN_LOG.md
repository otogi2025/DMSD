...
itsuki 拍板把外出申请从「事前审批」改成「事后确认」：学生提交那一刻就能出门，老师的「確認」只是事后留记录、不是放行开关。安卓跟 iOS `fd82164` 同日 1:1 对齐。

> ⚠️ **本节初版写「安卓也只有演示版有外出、生产版还没接 `/api/v1/outings`」——那是错的**（2026-07-22 当日核查后订正）。安卓**根本没有** iOS 那种 `#if DEMO` 编译开关（`ScheduleScreen.kt:69` 和 `ApiClient.kt:20` 的注释都写明了这点），业务数据一律走真接口，外出的假数据路径从来就不存在（`MockData.kt` 里搜不到任何外出相关内容）。调用点：`ApplicationsScreen.kt:81`（列表）/ `ApplicationDetailScreen.kt:99,157`（详情、撤回）/ `ApplyNewScreen.kt:153`（提出），2026-07-20 对齐工单 B08 `9e2b3bf` 接通。所以本节的改动**全部直接作用于真实生产行为**。
...
**待 itsuki 定的两处**（已在 §19 结清）：① `ApplicationDetailScreen.kt:164` 还写着「確認待ちの申請のみ取り消せます」，但徽章已经改成「承認不要」了，两句话对不上 —— 根子是「提交即生效之后，学生撤回还有没有意义」这个问题没定；② 「却下について」是子代理造的日语，没对照过宿舍手册用词。

**跨端对齐**：iOS 见 `IOS_DESIGN_LOG.md` §41。共用层语义 → `design/system_features.md` §7.2.7。

## 19. 撤回语义定案 —— 文案跟 iOS 逐字对齐（2026-07-22，commit `186060a`）

§18 留的尾巴 itsuki 拍板 **A —— 保留撤回功能，含义是「我不去了」**。安卓跟 iOS `186060a` 同 commit 对齐，改的是措辞对象不是功能：事前审批时代撤的是「一张待批的申请」，事后确认制下撤的是「这次外出本身」，所以按钮从「申請を取り消し」改成「外出を取りやめる」、进行中改「取りやめ中…」。

**这里有个必须小心的地方**：`ApplicationDetailScreen.kt` 里出寮届（358 行附近）和外出（538 行附近）的撤回按钮代码块**逐字节相同**，直接全局替换会连出寮届一起改掉 —— 出寮届仍是事前审批，「申請を取り消し」对它是准确的，不能动。定位靠的是外出那块上方新加的注释当锚点，改完 grep 复核出寮届那句仍在。

**409 文案（§18 待定 ① 的正解）**：旧文案「確認待ちの申請のみ取り消せます」在新语义下自相矛盾 —— 徽章已经是「承認不要」，界面上没有「確認待ち」这个词。改成跟 iOS 逐字相同的「先生が処理した後は取りやめできません。寮監に直接お伝えください」。后半句兜的是一个真实缺口：后端只允许撤 pending，万一老师手快、学生刚提交就被确认，学生就撤不了了，所以指向寮監。后端规则本次没动。通用异常兜底句也从「通信エラーが発生しました…」改成「外出の取りやめに失敗しました」（原来那句会把 409 之外的业务错误也说成网络问题）。

**§18 待定 ② 结清**：itsuki 判「却下について」这句自造日语不用送 jp-reviewer 审 —— 别再当遗留项重提。

**安卓没有演示版这回事**：iOS 这次顺带修了演示版详情页停在旧文案的问题（见 IOS_DESIGN_LOG §42），安卓不存在对应问题 —— 没有 `#if DEMO` 编译开关，业务数据一律走真接口。

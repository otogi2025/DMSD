...
### 3.5.9 登录页分组栏列数改为自适应（2026-08-08，commit `6243081`）

登录页把老师按权限分成四组并排（寮管理者 / 一般宿管 / 一般宿管＋夜学習 / 申請承認専用），列宽原本写死 `gridTemplateColumns: "1fr 1fr 1fr 1fr"`。在 iPad 竖屏 768 px 下每列只剩 153 px，「デモ（一般）先生」这类名字折成三行、「6 日前にログイン」折成两行。

改成 `repeat(auto-fit, minmax(200px, 1fr))`：

| 屏幕宽度 | 容器内宽 | 列数 | 每列宽 |
|---|---|---|---|
| 1024 px（iPad 横屏）| 928 px | 4（与改前一致）| 214.5 px |
| 768 px（iPad 竖屏）| 672 px | 3 | 207 px |

`200px` 这个下限由外层 `padding: "36px 48px"` 与 `gap: 20` 反推得出，**调大即回退**：改成 `220px` 时横屏容器只容得下 3 列，等于牺牲主力使用姿态去换竖屏。三列布局下第四组会落到第二行左侧，属 grid 正常行为，取舍上优先保证姓名不折行。

该缺陷早于本次视觉改造（`git log -S` 可追至 2026-06-14 目录改名之前），非改造引入。发现途径为在真实 768 / 1024 宽度下渲染生产站点比对，静态审查未能命中。
...
- **F6（当轮改法已于第三轮撤回，见下节 G1）**：H11 的 `_mergeBoardSnapshot` 对 `pending` / `override` / `health` / `exemptReason` 一律 `old.X ?? f.X`，而这四个字段 board 接口压根不返回，本地一旦挂上就永远清不掉——申请在别的终端批完了，座席仍挂「審査中」，老师按几次刷新都消不掉。当轮判定为两难而非纯 bug，折中为合并函数加 `keepLocalMeta` 参数：后台静默补拉保留、老师主动「座席を再取得」以服务器为准清一遍。根治始终是让 board 接口返回这四个字段（后端新工作，入 `admin/TODO.md`）。

検証：`npx tsc --noEmit` 0 错 / `npm run build` 绿 / F1 补 4 条 WS 票据回归测试、F2 补 3 条乐观锁测试。两组新测试都做过**反向验证**——把被测代码换回修复前版本重跑，该红的如期变红，确认不是恒绿摆设。

### 第三轮复审：双家背对背 + 一条自我推翻（commit `5ed3966` / `4719a5e` / `5609b1a`）

第二轮 6 条修完后再派两家背对背只读复审——cursor grok-4.5-high 与 codex gpt-5.6-sol，互不知对方存在。两家对 F1~F4 独立给出「修对了」，**零互相误报**；分歧只在结论口径：grok 判「可以上生产」，codex 判「不建议直接上」，各自多报一条对方没看见的低/中危。主会话逐条核实后三条全部属实并修掉。

- **F5 的跨场次残留**（低，grok 报；主会话在收到报告前已独立推出同一条，两处互证）：`boardSyncFailed` 只在补拉成功时清零，WebSocket effect 卸载时不清。一场点呼里补拉失败过，标记留到下一场；下一场刚连上、新补拉还没回来的那两百毫秒，故障横幅会先闪一下红字。在 effect 的 cleanup 里一并 `setBoardSyncFailed(false)`。
- **G2：两条重拉路径的请求世代号不共享**（中，codex 报）：F5 引入的世代号只存在于 WebSocket 那个 effect 的闭包里，老师主动点「座席を再取得」的路径根本不参与计数。于是「先发的静默补拉慢一步才失败」时，它仍能通过自己的守卫，把主动重取刚清掉的故障横幅重新点亮——座席其实是好的，老师看到红字。世代号提到组件级 `useRef`，两条路径共用同一个计数器，各自进入时 `++`、`await` 后比对。**教训：用世代号做「后到的旧请求不许写结果」这类守卫时，凡是会写同一份 state 的路径都必须共用同一个计数器**，闭包里各存一份等于没做守卫。
- **G1：F6 的分场景做法被推翻，改回一律保留**（中，codex 报）：codex 指出主动刷新会清掉**仍然有效**的徽章。核实四项事实后判定 codex 对、当天早些时候的 F6 折中错，予以撤回：① 这四个字段服务器不返回、映射里恒 null，所谓「以服务器为准」并没有一个服务器值可依，清掉是纯信息销毁而非纠正过期值；② 老师端 WebSocket 只推 `checkin` / `override` / `outstay_new` 三种事件，后端**没有任何事件**会在申请被批准或退回时通知老师端，所以「徽章过期」没有可感知的时机；③ 徽章变陈旧须是「同一场点呼期间别的终端刚好把这份申请批掉」，窗口很窄，而误清的代价是老师看不出学生已交外泊申请、去追一个有正当理由的人；④ `health` 全项目无任何写入点，是死字段。`keepLocalMeta` 参数删除，两个调用点回到统一规则。**教训：面对「保留还是清除」的两难，先问服务器对这个字段有没有话可说；服务器无话可说时，「以服务器为准」是个假选项。**

検証：`npx tsc --noEmit` 0 错 / `npm run build` 绿 / 后端全量 pytest 684 passed・1 skipped（基线 663 + 本轮新增 21 条）。两家复审员全程只读，收工时 `git status` 干净。

### 上线前审查的一条自我推翻：点呼首页统计改回演示 / 真账户双轨（commit `ff7b314`）

审查阶段把点呼首页的 4 张统计卡（本日実施 / 欠席者 / 審査待ち申請 / 警告リスト）和「最近の点呼」表格整块改成了「準備中」占位，理由是这些数字没有任何后端接口支撑、全是硬编码。理由本身没错，**但结论错了**——itsuki 2026-07-17 已就此拍板过（决策 5）：演示账号显示演示数据、真账户显示真数据或占位，**两者共存**。演示账号是给 App Store 审核员和宿舍管理员看的，界面必须有内容；真宿管老师则一个编出来的数字都不能看到。两个需求同时成立，不是二选一。更需要记的是：那条决策的批注当时就写在被删代码的正下方，仍然被当成纯假数据清掉了。

改回双轨的实装：后端 `TeacherOut` 新增 `is_demo`（前端没有这个标志就无法分轨，而在前端硬编码「哪个登录名是演示账号」更差），`api/types.ts` 的 `Teacher` 类型与 `App.tsx` 两条构造老师对象的路径（新登录 / 刷新页面时从会话存储恢复）跟着带上；两处取不到时都按 `false` 处理——旧会话存储里没有这个键，宁可少显示，也不能让真老师看到假数字。`RollCallLanding.tsx` 恢复统计卡与「最近の点呼」表格，包进 `isDemo` 分支，真账户仍走「準備中」，并在文件头写明双轨规则和「别再整块删掉」的告诫。演示表格里点呼名称顺带跟上本轮改动（去掉已删的「· 普通寮生」后缀，只剩朝 / 夜两种）。

**教训：删掉一段「明显是假数据」的代码前，先读它周围的注释**——项目里的历史决策往往就贴在它旁边，而不是在某份文档里等人去查。

检証：`npx tsc --noEmit` 0 错 / `npm run build` 绿 / 后端全量 pytest 684 passed・1 skipped（字段快照测试同步加 `is_demo` 后恢复基线）。

### 双轨范围补齐：趋势图与日付集計（commit `854126f`）

上一条只改了点呼首页的统计卡和「最近の点呼」，同类的另外两处仍停在一刀切占位——点呼首页的「最近 7 日 遅刻・欠席トレンド」（审查条目 web#45 改的）和检索页「日付検索」标签的全寮集計（web#40 改的）。两处的成因与统计卡完全相同：没有后端聚合端点，数字只能是硬编码，于是被整块换成「準備中」。按同一条双轨规则补上演示侧。

- **趋势图**：`TrendChart` / `Legend` 两个组件按删除前的样子恢复（柱状图、悬停显示当日遅刻・欠席数、点柱子跳该日记录），数据源改成组件内部生成而不是从 `App.tsx` 传入——原先的假数据挂在 `App.tsx` 上、写死 2026-04-15~21 七天。日期改为按 JST 今天倒推七天动态算：写死的话演示账号过几个月看到的是一张停在旧月份的图（iOS 演示数据已经踩过这个坑）。数值仍固定，保证演示画面每次一致。
- **日付集計**：新增按寮分行的演示表格（一寮 / 二寮 / 四寮 × 対象・出席・遅刻・欠席・外泊，末行合計）。各行数字自洽（出席 + 遅刻 + 欠席 + 外泊 = 対象），合計行由三寮求和算出而非另写一组数——写死的合計迟早跟上面的行对不上。**选日期不改变数字**：没有后端接口时，让数字随日期变化只是在假数据上再叠一层假象，反而更容易被误当成真统计。`SearchPage` 因此新增 `isDemo` 入参，由 `App.tsx` 从 `teacher.is_demo` 传入（这个 prop 在 web#39 那批曾作为死 prop 删过，现在有了真实消费点才加回来）。

顺带订正 `RollCallLanding.tsx` 文件头两处与代码不符的注释：一处写「见下面 DEMO_STATS / DEMO_RECENT」，而这两个常量并不存在（演示数据是内联 JSX）；另一处末尾写「本文件不再放硬编码假数字」，双轨恢复后已不成立。**注释指向不存在的标识符，比没有注释更费时间**——读者会先去找、找不到再怀疑自己。

検証：`npx tsc --noEmit` 0 错 / `npm run build` 绿。

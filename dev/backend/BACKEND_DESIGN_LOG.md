...
| 2026-07-21 | **审查 S5：后端 medium 后半 27 条**（568 修复计划第 5 场；分工=Opus 规划裁决+grok-4.5-high 下笔+分波文件不相交并行）：(1) **错误信封/日语** #9 全局 500 中文→日语、#10 限流 429 自写处理器走 build_error_body(RATE_LIMITED)+保留 Retry-After、#26 登录失败按方式分日语(学号→アカウント番号/邮箱→メールアドレス，时序安全)、#28 扣分 message 日语。(2) **寮边界测试补真覆盖** #56 XFF「最右一跳」注释纠错、#60/#62 补 selected_dorm=4 断 403 FORBIDDEN_DORM/FORBIDDEN(原走未选寮兼容路径证明不了 enforce)。(3) **逻辑** #24 编辑公告 reply_count 查真实未软删数(不写死0)、#41 renewal_start 软幂等守卫(进行中重复开闸不重打已完成者)、#19 password_reset 补 status==active 校验、#22 list_students 加 limit/offset SQL 侧兜底(形状不变)。(4) **设备端** #30 写/撤扣分前 with_for_update 锁学生行、#31 enroll 条件 UPDATE where enrolled_at IS NULL+rowcount 闭合竞态(错误码保持 422 契约)、#32 演示老师 list_devices 返[](设备无 is_demo)、#33 _find_session_for_checkin 时间窗下推 SQL(TZDateTime 绑定正确转 UTC、边界等价)。(5) **调度** #14 自动 start 上界 now<stored_auto_end 互斥(堵同 tick start+end 全员误缺席)、#16 start/end 用库存 scheduled_*(nullable=False)不用每 tick 重算(防配置变更分叉)。(6) **JWT** #21 create_access_token sub/role/iat/exp 保留 claim 强制写回防 extra 覆盖。(7) **学习族** #46 decide_absence 悬空 student→404 fail-closed、#47 patch_checkin 补扣分包 begin_nested savepoint 只吞 uq_demerit_source、#48 半节请假结算未实装(仅注释标已知缺陷，itsuki 两次拍板：点呼模型每晚单次点呼 uq_sc_date、真分段 feature 级留第二波)、#49 欠席届通知收件人按 effective_group+has_permission(C_STUDY,MANAGE)不硬筛 role、#51 submit_online 重叠校验前锁学生行。(8) **性能异步** #43 未读数三类独立 COUNT(不 limit、条件与 feed 逐条一致)、#45 WS 鉴权+心跳同步 DB 经 run_in_threadpool(各自开 Session 不跨线程/返标量)、#53 dorm_only 语义标注(全员皆寮生等价 all)、#54 broadcast_push 一次 IN 查令牌消 N+1。逐 diff 裁决抓修：grok 越界 enroll 422→409 撞测试(改回)、#19 message 照抄「番号变更」(改密码语义)、#47 术语先误 revert 经中文铁律 hook 点醒改回拍板词「夜学習」并统一 patch_checkin 两分支。相关测试累计 247 项 + 干净全量套件 640 passed 1 skipped(⚠️ 首跑 326 幻影 error=cursor --force 复审 agent 并发自跑 pytest 撞测试库，杀进程串行重跑得真值)。三方复审：grok CONVERGED(0 阻塞/0 重大)+opus 逐 diff(收 2 minor commit 06cea52/驳 1/留 5 保留意见)，composer-2.5 试 3 次撞 cursor resource_exhausted/连接中断(非月额度耗尽、与 grok 同属内含模型、疑暂时性可重试)→本场双票收敛收口。| [Mac-Opus 4.8] CC 规划裁决 + grok-4.5-high 下笔 + grok/opus 双票复审 |
...
| 2026-07-21 | **审查 S10：后端 low 100 条**（568 修复计划第 10 场；分工=Opus 4.8 规划+逐 diff 裁决验证、模型 8 批只读判 LIVE/FIXED/moot、grok-4.5-high 7 批文件不相交并行下笔）：103 候选经并行只读核实=**100 LIVE / 3 已修**(backend#109/#140/#151)。修复分类：(1) **真健壮性** #89 公告已读并发 IntegrityError→try/except 幂等；#100 `_find_session_for_checkin` max 键 naive/aware 混比 500→`_as_jst_aware` 归一；#106 `dorm_unit_set` 为 None `in None`→`(… or [])`；#108 outings confirm/get 学生悬空→404 fail-closed；#117 APNs Unregistered/BadDeviceToken 恒久失败→send_push 置 revoked_at 扫死令牌；#88 设备 WS 重复握手残留多连接→connect 先剔旧；#96 device_tokens 兜底 created 语义；#110 学生已读写前校验目标可见性；#116 空名簿 exempted_count 补齐四键；#127 创建者查不到返空受众不误推真实学生；#73 429→RATE_LIMITED 专用码；#74 信封判定强化 `ok is True/False`；#105 today_sessions latest 用 (checked_in_at,id) 元组；#90/#92 聚合收窄/过滤下推 SQL。(2) **注释语言 日→中** models/database/security/approval_chain/conftest/env.py/3 迁移；**UI 串 中→日** deps token→トークン、bus/incidents/lost_found/rollcall/student_profile 文案、schemas 登录、email 承认→承認。(3) **死代码** 删 CROSS_DORM_ROLES/ADMIN_ROLES/_EDIT_ROLES(bus,events)/_ROSTER_ROLES/tannin_token 等零引用。(4) **冗余索引** models.py 6 列 index=True 去除(复合索引最左前缀覆盖)+ 迁移 c1d2e3f4a5b6 同步删 demerit 单列 create/drop。(5) **测试卫生** 断言收紧(双码→单码/!=403→白名单)、命名订正(test_403_*→实况)、过时 docstring；`_kisei_body`+挑人查询抽共享（新增 `tests/helpers_applications.py` + `app/services/student_picker.py`，front_desk/discipline 复用）。裁决抓修 grok 2 处回归：push `_send_via_apns` 改 3 元组破坏 test_push_apns（补 fake `.json()`+解包）、#163 断言裸 hour 跨时区比（改瞬时差）。**backend#79**(_settle_absent 抽 services) defer——scheduler 已函数内 import 避循环、无循环 bug 纯洁癖，函数含原子领取/行锁不变量移动风险高（记 carry-forward，S14 对 itsuki 明示）。全量 pytest 640 passed 1 skipped。双票复审 grok+opus 背对背只读收敛：**grok 抓出 opus 漏的 1 重大**——#117 把 `_dispatch_one` 改 3 元组后，生产推送主路径 `broadcast_push`(student_audience，公告/巴士/行事「学生に通知」走它，非仅测试用的 send_push)仍按 2 元组解包→ValueError 被吞成 dispatch failed、成功投递到不了、死令牌不撤（pytest 未抓因推送失败静默）；修：3 元组解包 + 补撤死令牌逻辑与 send_push 一致。另 2 次要：#110 announcement 分支补 scope 可见性守卫(与 bus/event 同口径)；#72 迁移侧 4 表冗余单列索引 drift 判无害接受(models 真值源已干净，下次 autogenerate 或上线前合并迁移清，记 carry-forward)。复审修复 commit 见 S10 后续。 | [Mac-Opus 4.8] CC 规划裁决 + grok-4.5-high 下笔 + grok/opus 双票复审 |
...
| 2026-08-01 | **老师网页上线前审查的后端侧 2 条（C3 时区归一化 + H4 事案寮过滤）**（三家 AI 背对背审查 33 条，itsuki 拍板修 3 阻断 + 10 高危；其余 11 条在 `dev/teacher_web/`，见 `WEB_DESIGN_LOG.md` 同日条）：① **C3 行事/巴士时刻偏 +9 小时**（commit `2dc4a26`）——老师网页 `<input type="datetime-local">` 发的是无时区串（`2026-08-05T19:00:00`），`database.py` 的 TZDateTime 规则是「无时区值一律当 UTC」，于是老师填的 19:00 存成 UTC 19:00 = JST 次日 04:00，行事和巴士时刻全线偏 9 小时。itsuki 拍板**改后端不改前端**（客户端有 iOS / Android / 网页三个，全都改一遍容易漏；后端一处收口更稳）：`events.py` / `bus_routes.py` 各加 `_coerce_jst()`——`tzinfo is None` 则 `replace(tzinfo=ZoneInfo("Asia/Tokyo"))`，已带时区的原样保留，None 跳过；覆盖 `events` 的 POST `start_at`/`end_at`（`_check_time_range` 用归一化后的值比较，否则同一请求里混着 naive 和 aware 会 TypeError）+ PATCH 的 `provided` 字典，`bus_routes` 的 POST `schedule_at`/`arrival_at` + PATCH 同样。② **H4 事案记录缺寮过滤**（commit `17052b9`）——`incidents.py` 是全后端最后一个完全没有寮过滤的 router，男寮当值老师能读到女生事案的全文和真名（事案正文常含具体违规经过，比排行榜的分数敏感得多）。新增 `_parse_involved_uuids` / `_assert_student_in_dorm` / `_assert_incident_in_dorm` / `_filter_incidents_by_dorm`，5 个端点（POST/GET 列表/GET 详情/PATCH/DELETE）全覆盖，403 码 `FORBIDDEN_DORM`。判定口径 =「涉及学生**全部**在可见寮内才放行」，与 discipline / study / rollcall 同款；无涉及学生的事案不属于任何寮、不受限。`_build_student_name_map` 额外按可见寮裁姓名（纵深防御——正常 API 路径走不到，因为事案整条已先被拦掉）。③ 回归测试 `TestIncidentDormBoundary` 14 件（commit `7e8be68`），5 个端点的「该拦」和「不能误伤」两侧各测一遍 + 一条白盒直调姓名映射。**反向验证**：把 `incidents.py` 换回修复前版本重跑，7 条「该拦」的测试全部变红，换回来 35 passed——测试确实守得住这个漏洞。验证：全量 pytest **663 passed / 1 skipped**（⚠️ 必须串行跑——子代理并行跑会抢同一份 `test_tomoshibi.db`，本场踩到过一次假大红 49 failed/263 errors，等并发结束后单独重跑即全绿）。| [Mac-Opus 5] CC 裁决验证 + grok-4.5-high 下笔 + sonnet 5 补测试 |
| 2026-08-01 | **第二轮复审的后端侧 2 条（WS 票据丢当班寮 + 手动设定合计点乐观锁）**（13 条修完后派 codex gpt-5.6-sol 只读复审，报 6 条，主会话逐条读代码核实全部属实；前端侧 4 条见 `WEB_DESIGN_LOG.md` 同日「第二轮复审」小节）：① **F1 WS 票据丢当班寮**（commit `015284b`）——老师登录时选的当班寮 `selected_dorm` 进了登录令牌、REST 接口按它算可见范围，但 `auth.py` 的 `issue_ws_ticket` 换 WebSocket 短时票据时 `extra` 里只放了 `purpose`，把它丢了；`ws.py` 的 `_load_teacher_for_ws` 于是回落读老师档案里固定的 `assigned_dorm`，`ws_manager.broadcast` 再按它裁广播。后果：档案男寮、今晚代班女寮的老师，点呼大屏显示「已连接」却收不到一条女寮签到，反而收到男寮的——**这是服务器按错寮裁广播，不是前端显示问题**。修法：`auth.py` 把 `getattr(teacher, "_selected_dorm", None)` 透传进票据（为 None 时不放这个键，与登录令牌写法一致）；`ws.py` 的 `_load_teacher_for_ws` 新增 `selected_dorm` 参数，赋给 `teacher._selected_dorm` 后调 `deps.dorm_units_for_teacher()` 算可见寮列表，再折回 `ws_manager` 认识的单一寮值（`[1,2,4]`→`None` 不限制 / `[1,2]`→`1` / `[4]`→`4`）。折回而非把列表塞进 `_TeacherConn`，是为了不动 ws_manager 字段和 6 个既有广播过滤测试；复用 deps 口径顺带修好 op / 申請承認専用 组走 WS 时按档案寮被误裁的既有问题。前端无需改动——换票据用的就是登录令牌，后端自己读得到。**这是 memory `project_backend_jwt_bypass_entrypoints` 记的「5 处自解 JWT 不走 deps 的鉴权入口，改鉴权规则必须逐个覆盖」第三次翻车**（前两次：临时账户过期未覆盖 WS、7-18 设备令牌世代校验未覆盖 WS）。回归测试 `TestWSTicketCarriesSelectedDorm` 4 件：票据带上寮 / 未选时不带该键 / 核心单测「当班寮优先于档案寮」四种输入 / 端到端查 `manager._conns` 按 4 注册。② **F2 手动设定合计点的 TOCTOU 窗口**（commit `20fd36a`）——`POST /discipline/manual` 是绝对值覆盖语义（填 6 分 → 记一条「6 − 当前分」的差值），前端提交前虽已改为现查当前分核对（同日 H5），但现查到 POST 到达之间仍有几百毫秒，点呼迟到等自动扣分落在其中会被静默抵消。纯前端补不上——判断与写入必须在同一把锁内。`DemeritManualIn` 新增可选 `expected_current_points: float | None`，`create_manual_demerit` 在 `with_for_update()` 行锁之后、算 `delta` 之前比对，不一致抛 409 `POINTS_CHANGED` 且一行不写；字段可选，iOS / Android 老客户端行为不变。**浮点相等比较在此安全**：`DemeritEvent.points` 是 `Float` 列、`current_month_total_points()` 显式 `float()` 返回（PostgreSQL 不会返回 `Decimal` 导致恒不相等），且扣分是 0.5 步进、在 IEEE754 下精确可表示。回归测试 `test_discipline_optimistic_lock.py` 3 件（一致写入 / 不一致 409 且不写库 / 不传兼容），做过反向验证：回退 `discipline.py` 后「不一致该拒绝」如期变红。| [Mac-Opus 5] CC 裁决 + F1 亲修 + F2 grok-4.5-high 下笔后 CC 五步独立验证 |

| 2026-08-02 | **`TeacherOut` 增加 `is_demo` 字段**（老师网页点呼首页统计按账号类型分轨的后端侧；前端侧见 `WEB_DESIGN_LOG.md` 同日条）：`Teacher` 模型早有 `is_demo` 列，但 `TeacherOut` 一直不下发，前端因此完全没有演示账号的概念——上线前审查把点呼首页那些没有接口支撑的统计数字一刀切改成「準備中」，等于推翻了 itsuki 7-17 拍板的 is_demo 双轨（演示账号看演示数据、真账户看真数据或占位，两者共存）。要恢复双轨，前端必须知道当前登录的是不是演示账号；替代方案「前端硬编码哪个 login_id 是演示账号」明显更差（登录名一改就失效、且把账号策略散进前端）。故 `TeacherOut` 增加 `is_demo: bool = False`。**该字段只是显示开关，不承担隔离职责**——真正的数据隔离仍在既有的 `demo_scope_for_teacher`（真老师查不到 `is_demo` 学生），前端拿到 True 也只影响自己显示什么。同步更新 `test_schema_field_snapshot` 期望集；改前已复查 iOS / Android 学生端源码，两端都不解码 `TeacherOut`，无需跟改。全量 pytest 684 passed / 1 skipped。设计文档 `design/system_features.md` §7.20 的「双层防御」表同步补上老师层这一行（原表只有学生层 + 注册码层）。| [Mac-Opus 5] CC |

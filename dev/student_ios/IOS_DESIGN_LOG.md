...
> **最后更新**：2026-06-14（v0.23.9 审查 Tier1 修复批：**生产 NFC 点呼不再本地伪置 done** → `recordCheckin()` 整段进 `#if DEMO`、生产状态全由后端 `my_checked_in_at` 驱动（消除被每秒刷新覆盖的竞态）；防抖 8 表单 / 合同下载走统一 APIClient / 16 处错误文案不暴露内部 / PDF 先验大小 / 上传不吞 401。技术细节见 `CHANGELOG.md` v0.23.9）/ 2026-05-28（§3.3 + §3.9.2 注册页 demo 默认空 + 送后端转数字码修真 bug）/ 2026-05-27（§实装进度速查表 5 个 🟡 全 ✅）。早些更新：2026-05-21 A-029 / 2026-05-03（§3.9.2 注册 flow 5→6 step + §3.12「登録コード入力」）/ 2026-05-02 §11.9 I1/I2 / 2026-04-22 晚 Round 1
...
- 番号 / メール 两个 tab + 密码（`AuthAPI.loginStudent(studentNo:)` / `loginStudent(email:)` 分开发，body 不含另一字段的 null）
- **永久保持**（直到主动 ログアウト）
- 成功登录 → 错误 counter 清零（N3 ✅）
- 首次启动 → 没 session → 注册 flow
- 后续启动 → 有 session → Home
- **2026-07-19**：恢复「メール」tab（原先 §32 因后端未实装而隐藏；现后端已支持邮箱登录）
...
**⚠ 权威源是 `design/system_features.md §3`。这里只是 iOS 视角的摘录 + App 专有的界面规格。**
...
**⚠ 权威源是 `design/system_features.md §4`。**
...
**⚠ 权威源是 `design/system_features.md §5`。**
...
**⚠ 权威源是 `design/system_features.md §7.16`。这里只是 iOS 视角的界面规格。**
...
**双端同步**：上架版 fork + 主项目（`dev/student_ios/v1/`）同步改完 2026-05-07（fork 已退役，主项目为唯一开发线）
...
`dev/student_ios/v1/TomoshibiApp/Features/MyPage/MyPageStubs.swift` — `MyLandingView`（line 51〜）。
...
>
> **单 repo**（2026-05-06 退役独立 repo / 2026-05-21 C-012 清理）: Swift 实装直接在 `dev/student_ios/v1/TomoshibiApp/`，跟 backend / Android / Web / 点呼机 全在 DMSD 单 repo 里。原跨 repo 同步规则已废。
>
...
**実装**: `HomeStubs.swift` line 875 + `MyPageStubs.swift` line 211〜 + `design/system_features.md` §7.6.2。
...
> spec 共用層 = `design/system_features.md §7.15`。本节 = iOS 専属 UI 仕様 + 実装ファイル对応。
...
| spec | `design/system_features.md §7.15.11` 表更新 | — |
...
> 生效真值在 `dev/student_ios/v1/project.yml`（这里是人读的对照登记）。Team ID 不是密码、是半公开标识，进公开仓库属正常。
...
## §24 [2026-06-15] 日语 UI 文字母语者全量复审 — 10 处自然度修复

> 起因：itsuki 问「所有日语都自然了吗」。此前 6-13 审查 + Tier2/Tile3 修复只覆盖「被标记」的字符串，从未做过从头逐条的母语者普查。本节是独立于旧标记的全量复审结果。

### §24.1 方法
全量扫 28 个含假名的 Swift 文件、793 条日语 UI 字符串。工作流：16 初审代理（11 覆盖全部文件 + 5 对最显眼大文件二次「语感」复审）→ 去重 → 每条疑点独立对抗终审（初审倾向多报、终审默认驳回，两头夹逼校准；自动滤掉「審査中です/なし」等已确认自然的误报）。32 条疑点 → 10 真问题 / 22 误报。
...
| CommunityStubs:762 | 拾得場所 | 場所 | 兼「落とし物」时语义反 |
| AuthStubs:1864 | （直接）アカウント番号で… | 補「メールでのログインは現在ご利用いただけません。」 | 命令式缺前提 |
| SEED:95 | 駅前·タクシー予約 | 駅前·買い物 | 演示数据结构不搭 |

### §24.3 误报里的反向确认
终审替上轮 Tier-C 改的 `判定の変更はされていません`（MyPageStubs）背书「自然，保持原样」；「電波を確認してください」两轮 AI 判得不一致 → 判为微妙语感非真错，未改。
...
## §27 [2026-06-16] 日语 UI 自然度全量审查施工 — 以宿舍真实しおり当基准，16 文件 149 处

> 起因：itsuki 要把整个 iOS app 的**所有日语** UI 再做一遍自然度审查，并第一次把**真实宿舍手册**《2026年度生徒寮のしおり》当对照基准（不是凭 AI/直觉语感）。§24（6-15）只在 793 条里凭语感扫出 10 处；本次拿手册逐词对照，确认 94 条问题、落地 149 处编辑。审查 + 施工图在前一会话产出（`admin/handoff/iOS日语自然度审查_报告+施工图_2026-06-16.md`），本节是施工落地记录。

### §27.1 itsuki 拍板的 3 个决定
1. **晩自習 → 夜学習**：app 全用「晩自習」(80 处)，手册一律「夜学習」，连 app 自己的演示公告都写「夜学習」(自相矛盾)。改回「夜学習」。英文代号 `study`/`studyAbsence` 不动、接口契约零变化。
2. **罰則清掃 保留命名**：不用手册的「環境整備」覆盖产品名，只统一被写反的词序「清掃罰則」→「罰則清掃」。
3. **学号统一「アカウント番号」**：所有「学籍番号」→「アカウント番号」(不采手册的「生徒番号」，按 app 自己体系)。

### §27.2 施工方式
工作流 13 代理分文件并行（每代理只改自己那几个文件、互不冲突）→ 各自返回逐条 diff → 主会话 grep 全树复验 + 亲自双 scheme 编译。共 16 个 swift 文件 149 处编辑。

### §27.3 修正分类（节选，全量见施工图）
| 类型 | 例 | 性质 |
|---|---|---|
| 中文漏出（硬伤）| HomeStubs `Text("請假")` → 欠席届；SEED/MyPage 罚扫状态「退回」→「差し戻し」| 必修 |
| 晩自習→夜学習 | 全 app 74 处（UI 串 + 注释，含 AppStore 29 / MyPage 16 / Home 13）| 决定 1 |
| 晩点呼→夜点呼 | SEED 点呼履历 + MyPage:1077 生产映射 | 手册对齐 |
| 学籍番号→アカウント番号 | Home 5 处 + AppStore toast | 决定 3 |
| 保証人→保護者 | ApplyStubs 外泊/帰省同意书 5 处 | 手册对齐 |
| 認証コード→登録コード | Route + AuthStubs 注册第 5 步屏 4 处 | 注册流程统一 |
| 清掃罰則→罰則清掃 | Route + MyPage 词序 | 决定 2 |
| 用词不当 | 寮務課長(承认链)/食事申告→申し込み/予想経費→概算経費/学生納付金→寮費/礼儀を守って→ルールを守って/お問い合わせ→申し出て | 自然度 |
| 排版 | 半角中点「·」→全角「・」、中文逗号「，」→「、」、半角省略号「...」→「…」、名词数字间去半角空格——仅日语 UI 串内改，中文注释里的中文标点不动 | 自然度 |

### §27.4 主会话施工时的判断（不盲从施工图 / 代理）
- **AppStore:1505 回退**：代理按施工图把演示通知「寮務課長まで承認が進みました」改成「寮務部長」。但 `寮務課長` 是 app 审批链里的**真实独立角色**（StayListStubs `case dormChief = "寮務課長"`，对后端 `approver_role` 契约；留学生链 担任/国際交流部長/寮務課長/寮務部長/管理係 二者并存）。施工图理由「しおり 无課」站不住——app 自己的角色模型就用課長。只改一处会让全 app 角色名不一致，已回退。
- **施工图漏列的同类占位由 grep 补全**：認証コード 不止 Route（AuthStubs 实际注册屏还有 3 处）、清掃罰則词序不止 MyPage:1570（:1658/:1735 也是 UI）、晩点呼不止 SEED（MyPage:1077 是生产 session_type 映射）。
- **SEED specials 字典 key 联动**：点呼履历循环 `["朝","晩"]`→`["朝","夜"]` 后，specials 查找 key「2026-04-20:晩」必须同步改「:夜」否则两天的「欠席」特殊状态静默失效——保持原行为的必要联动。

### §27.5 留给 itsuki 的命名决策（本次未动）
- **寮務課長 / 国際交流課長**：手册无「課長」只有「部長」，但这些是审批链结构角色（对后端契约），是否按手册调是 app-wide 命名决策，同「減点→指導」性质，待通盘拍板。
- **減点 → 指導**：手册走「指導/指導対象」非分数化阶梯（0 处「減点」），DMSD 内部建模是扣分制（DemeritEvent），面向用户措辞改不改是命名哲学决策，未碰。
- **欠席申請 vs 欠席届**：TopRollBar 的 tap 提示已按施工图改「欠席届」，但 HomeStubs:711 按钮 label「欠席申請」是动作语，未统一，留你定。

### §27.6 跨端联动待办（记 TODO §B）
本次只改 iOS。itsuki 6-15 是「全端」把「学習→晩自習」(后端 commit `684cd5c` 等)。现 iOS 改回「夜学習」，**其余 4 端（后端用户可见文本 / 老师网页 / 安卓 kind label / spec+设计文档）还停在「晩自習」**——全端再统一一次待 itsuki 拍。
...
> ⚠️ **点呼入口占位已于 §34（2026-07-17）反转解除**；**邮箱登录 tab 隐藏已于 2026-07-19 反转恢复**（后端实装邮箱登录后）。
...
- ~~**邮箱登录 tab 隐藏**~~ → **已恢复（2026-07-19）**：`LoginView.modeTab` 重新显示「メール」；`tryLogin` 删「ご利用いただけません」早退，走 `AuthAPI.loginStudent(email:)`。
...
## §35 恢复邮箱登录 tab + 接通后端（2026-07-19）

- `LoginView.modeTab` 加回「メール」（反转 §32 隐藏）。
...
itsuki 拍板把外出申请从「事前审批」改成「事后确认」：学生提交那一刻就能出门，老师的「確認」只是事后留记录、不是放行开关。本次改的是文案与入口闸，不涉及网络层。

> ⚠️ **本节初版写「iOS 只有演示版有外出、生产版还没接 `/api/v1/outings`」——那是错的**（2026-07-22 当日核查后订正）。错误源头是 `admin/TODO.md` 一条 6-04 写下、之后一直没更新的「⏳ 待做」条目，实装计划书和会话汇报连续继承了它。实况：外出的 4 个学生接口早在 2026-06-14 顶层目录改名之前就接通了，调用点在 `ApplyStubs.swift` 134 / 1908 / 2737 / 2755 行，全部位于 `#else` 生产分支内。下面描述的所有改动因此**同时作用于演示版和生产版**，不是只改演示版。
...
**待 itsuki 定的两处**（已在 §42 结清）：① 子代理自作主张把 pending 徽章颜色从 `.warn` 改成了 `.accent`（理由说得通：不再是「警告等待」了，但这是没授权的视觉改动）；② 却下卡片标题「⚠ 却下されました」是子代理造的日语，没对照过宿舍手册用词。

**跨端对齐**：Android 同日同步（见 ANDROID_DESIGN_LOG）。共用层语义 → `design/system_features.md` §7.2.7。

## §42 撤回语义定案 + 演示版详情页补上事后确认制文案（2026-07-22，commit `186060a`）

§41 留了个尾巴：提交那一刻就能出门之后，「撤回」还有什么意义？itsuki 拍板 **A —— 保留这个功能，含义是「我不去了」**，两端文案统一。

**改的是措辞对象，不是功能**：事前审批时代撤回的对象是「一张待批的申请」，所以写「申請を取り消し」；事后确认制下学生撤的是「这次外出本身」，所以改成「外出を取りやめる」（进行中「取りやめ中…」、成功提示「外出を取りやめました」）。**出寮届那侧一个字没动** —— 它仍然是事前审批，`ApplyStubs.swift:2394` 那句「申請を取り消し」对它是准确的。

**409 文案原来是自相矛盾的**：旧文案写「確認待ちの申請のみ取り消せます」，可 §41 已经把 pending 徽章改成了「承認不要」——界面上根本不存在「確認待ち」这四个字，学生看到只会更懵。改成「先生が処理した後は取りやめできません。寮監に直接お伝えください」。多出来的后半句是给一个真实缺口兜底：后端规则是只有 pending 能撤，事后确认制下老师通常等学生回来了才批量确认，但万一老师手快、学生刚提交就被确认，学生就再也没法说「我不去了」。**后端规则本次没动**（改它要重新想清楚「已确认的外出能不能撤」这套账），先给学生留一条人工出路。

**顺带查出演示版才是漏的那个**（itsuki 要求核实「不能只有演示版有新改动」，实测方向正相反）：写脚本解析 `ApplyStubs.swift` 的 `#if DEMO` / `#else` / `#endif` 嵌套逐行判分支，结论是生产分支 0 处遗漏 —— §41 和本次的新文案全都写在共用代码里。但脚本反过来暴露了一件事：**演示版根本走不到 `OutingDetailView`**。`"outing:"` 这个 id 前缀只在 `#else` 生产分支里拼装（134 行），演示版点开外出走的是另一条路 `otherDetailBody` 读 SEED 假数据，那条路上的文案停在 2026-06-04 的旧语义：

- 进度第 2 步还写着「先生の確認待ち」/「担当の先生が確認します」→ 改成跟生产路径逐字对齐的「先生の記録待ち」/「確認は記録のためで、外出は可能です」
- 详情状态徽章用的是出寮届那套 `statusPair`（approved 显示「承認済」）→ 改成按 type 分流用 `outingStatusPair`（显示「確認済」）。列表页 254 行早就分流了，详情页一直漏着，结果同一屏里列表徽章写「確認済」、点进去写「承認済」

演示版是拿给宿舍管理员看的，语义必须跟真机一致，不能停在被推翻的旧说法。

**§41 那两处待定同时结清**：itsuki 判 pending 徽章颜色改 `.accent` 保留、「⚠ 却下されました」这句自造日语不用送审 —— 别再当遗留项重提。

验证：`xcodegen generate` 后 xcodebuild 双 scheme（TomoshibiApp + TomoshibiAppDemo，iPhone 17 Pro）均 BUILD SUCCEEDED；脚本复核 4 句核心文案在两个分支全覆盖。

**跨端对齐**：Android 同日同步（见 ANDROID_DESIGN_LOG §19）。共用层语义 → `design/system_features.md` §7.2.7。

## §43 上线前只读审查的收尾修复（2026-07-22，commit `a2a4b15` / `b7141ca`）

上线前派 grok4.5 做只读审查，抓到 §41/§42 都漏掉的同一类病：**改文案时只顾着「学生刚提交」这一态，忘了另外三态**。

**提交完成页仍在叫学生等审查**（`a2a4b15`）。学生按下「提出する」跳到的第一屏是通用的 `ApplyDoneView`，文案写「審査完了時に通知でお知らせします」+ 一张「審査時間の目安 1〜2 時間」卡片。上一屏刚说完「承認不要、提交即可出门」，下一屏当面推翻。外出单独分流：第二句直接复用详情页那句已定稿的「確認は記録のためで、外出は可能です」，不另造日语；「審査時間の目安」整张卡不出现（写「0 時間」之类反而更像在暗示要等）。

**演示版详情页把外出当出寮届处理**（`b7141ca`）。§42 修好了演示版的进度文案和徽章，但同屏下方的动作区一直没分流，三处错：

- 进度步骤只判 `approved` 与「其他」→ `rejected` / `withdrawn` 一起落进「先生の記録待ち」+「外出は可能です」，等于告诉学生一次被却下、或自己取消掉的外出还能去。改成四态各走各的（確認 / 却下 / 取消 / 記録待ち），跟生产 `OutingDetailView.steps` 一一对应
- 却下横幅写死了出寮届的话术「帰寮予定時刻が門限（22:00）を過ぎています。外泊申請として再提出してください」，还配一个「内容を修正して再提出」按钮 —— 外出的却下只是记录 + 通知，没有「改一改再交一次」这条路。改成照抄生产的却下卡（「却下されました」+ 寮監指引），按钮对外出不出
- 取消按钮仍是「申請を取り消し」→ 改「外出を取りやめる」，跟 §42 定的 A 案对齐

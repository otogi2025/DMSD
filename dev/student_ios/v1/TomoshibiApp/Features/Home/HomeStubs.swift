...
//      + FeedbackSheet / HealthSheet / AbsenceSheet / OtherSheet / RenewStudentNoSheet
//      （原本住在这里的 RollcallSheet / StudyCheckinSheet 已搬去 HomeCheckinDemoSheets.swift —— 它们现在只有演示版用）
...
struct HomeView: View {
    ...
    // active/late/done 时：切换到点呼英雄显示（大字「22:00 = 遅刻になる時刻 / 遅刻 / 時間内」+ 欠席申請 / 体調報告 按钮）
    //                      本月分数退避到右下角小字

    /// 点呼英雄区（active / 遅刻 / done / absent 那套大字）是不是正在占用这张卡。
    /// 夜学習模式和 idle 各有自己的内容，卡片配色不能跟着点呼状态走。
    private var showsRollHero: Bool {
        if app.displayUser.isStudyTarget && (app.studyState == .upcoming || app.studyState == .active) {
            return false
        }
        return app.rollState != .idle
    }

    /// 签到成功完成态（按时 / 免除）—— 卡片整张转绿的开关。
    /// 遅刻虽然也是 .done，但那不是「成功」，卡片保持琥珀色、大字仍是红的，不给绿。
    private var isCheckinSuccess: Bool {
        showsRollHero && app.rollState == .done && app.checkinKind != "遅刻"
    }

    private var pointsCard: some View {
        // JSX ink：#5c3410（深褐）；签到成功卡片转绿后，深褐字压在绿底上发脏 → 换成深绿墨色
        let deepBrown = isCheckinSuccess ? Color(hex: 0x17452F) : Color(hex: 0x5C3410)
        return ZStack {
            // JSX: radius 22 / padding 20 22 / amber gradient
            RoundedRectangle(cornerRadius: 22, style: .continuous)
                .fill(cardGradient)
                .shadow(
                    color: (isCheckinSuccess ? Color(hex: 0x4A9478) : Color(hex: 0xD4A547)).opacity(0.24),
                    radius: 20, x: 0, y: 6
                )
    ...
    /// absent 时显红色渐变，签到成功时显绿色渐变，其余情况显 amber
    private var cardGradient: LinearGradient {
        // 签到成功整张卡转绿（2026-07-29 itsuki 真机反馈「画面没有变成成功，没有变成绿色」）。
        // 跟 absent 转红是同一套设计语言：状态变了，一眼就能从卡片底色看出来，不用读字。
        if isCheckinSuccess {
            return LinearGradient(
                stops: [
                    .init(color: Color(hex: 0xE6F7EC), location: 0.0),
                    .init(color: Color(hex: 0xA6DCBC), location: 0.55),
                    .init(color: Color(hex: 0x5FB58C), location: 1.0),
                ],
        ...
        if let _ = app.nextStudyTap {
            HStack(spacing: 10) {
                Button {
                        ...
                        NFCCheckinLauncher.start(type: .study, app: app) {
                            // 写入成功 = 本地物理确认（非权威判定），更新上方 2 次打卡进度圆点
                            app.recordStudyTap()
                        }
                    #endif
                } label: {
    ...
    /// active: 大字显「時間内」截止钟点（如 22:00）/ late: 红色迟到 / done: 绿色时间内
    @ViewBuilder
    private func heroStatus(deepBrown: Color) -> some View {
        switch app.rollState {
        case .active:
            // 大号数字从「还剩多少分秒」改成「几点之后算遅刻」的钟点 —— 学生不用再心算。
            // 钟点取不到（后端没给场次 / 演示版反推不出）时回退成原来的倒计时，不留空白。
            let deadline = app.rollOnTimeDeadlineText
            if app.rollCountdownSec <= 0 {
                heroBlock(
                    caption: deadline.map { "\($0) を過ぎました" } ?? "今回の点呼",
                    big: "遅刻",
        ...
        case .done:
            ...
            heroBlock(
                ...
                bigColor: isLate ? T.danger : Color(hex: 0x17452F),
                captionColor: deepBrown.opacity(0.7),
                subColor: deepBrown.opacity(0.8),
                // 成功态在大字左边加一个打勾圆章 —— 卡片转绿之外再给一个「一眼可辨」的信号，
                // 尤其对不读日语的人（App Store 审核员）也能看懂这是完成了。遅刻不给勾。
                bigIcon: isLate ? nil : "checkmark.circle.fill"
            )
        case .idle:
            EmptyView()
        }
    ...
    private func rollActionButton(icon: String, label: String, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            ...
            // 字色跟卡片墨色走：卡片转绿后还留深褐字会显脏、跟卡上其它字也对不上
            .foregroundStyle(isCheckinSuccess ? Color(hex: 0x17452F) : Color(hex: 0x5C3410))
            .frame(maxWidth: .infinity)
            .frame(height: 40)
            .background(
                RoundedRectangle(cornerRadius: 12, style: .continuous)

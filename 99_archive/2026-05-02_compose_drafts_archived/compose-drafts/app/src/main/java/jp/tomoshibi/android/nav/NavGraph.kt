...
fun TomoshibiNavGraph(navController: NavHostController) {
    NavHost(
        ...
        exitTransition = { slideOutHorizontally(tween(320)) { -it / 6 } + fadeOut(tween(280)) },
        popEnterTransition = { slideInHorizontally(tween(320)) { -it / 6 } + fadeIn(tween(280)) },
        popExitTransition = { slideOutHorizontally(tween(320)) { it } + fadeOut(tween(280)) }
    ) {
        // ── auth flow ─────────────
        ...
        // ── second-level ──────────
        composable(Route.ApplyNew.path) { ApplyNewScreen(navController) }
        composable(

...
def delete_my_account(
    ...
    """Apple App Store 5.1.1(v) 强制要求 — app 内账号删除端点。

    软删除：把 students.status 切到 'deleted'（不物理删，保留历史）。
    删除后 get_current_student 会返回 'ACCOUNT_INACTIVE' → 登录 + API 调用全部拒绝。
    同时写 AuditLog 留删除事件痕迹。
    """
    ...
    db.add(
        models.AuditLog(
            ...
            payload={
                "student_no": f"{student.grade_code}{student.class_code}{student.seat_no}",
                "name": student.name,
            },
        )
    )

...
> ⭐ **已并入收尾流程（itsuki 2026-06-11 拍板）**：每次收尾走核对表第 13 项「版本判定」— 当场过 §2 决策树，该升当场升（不再攒 commit 回头补标签），不该升报告理由。多会话叠加：看 `CHANGELOG.md` 顶部当前最新版往上叠。CC 否决权照旧。见 session-wrap §7.5.1。
>
...
> ⭐ 第 8 项后补 — 三端客户端版本号实际每次大版本都在同步改，但本清单一直没列过，曾因此漂移（一次重排后三端版本号停在旧值没人发现）。

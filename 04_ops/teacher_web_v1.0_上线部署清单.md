...
> 适用：后端 `03_dev/backend/v1`（FastAPI + PostgreSQL）+ 老师网页 `03_dev/teacher_web/v1`。

## 1. 环境变量（写进 `03_dev/backend/v1/.env`）
...
| 变量 | 说明 |
|---|---|
| `TEACHER_WEB_DIR` | 老师网页静态文件目录（如指向 `03_dev/teacher_web/v1/src`）。设了后端就在 `/teacher` 挂载 serve 网页（同源部署）；不设则跳过（网页另外单独 serve） |
...
- **NFC 防代刷后端零实装** —— 见 `02_design/NFC防代刷_后端立项施工计划.md`。当前点呼签到没有 nonce 校验 + ECDSA 验签 + 卡绑定，真防代刷要单独立项做。

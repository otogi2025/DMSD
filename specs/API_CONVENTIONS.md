...
更新时间：2026-07-17（§1 标已实装 + OpenAPI 失真声明）。此前：2026-07-07（§1 补实装状态标注 —— {ok,data} 包络已定稿待实装、排期 = App Store 占位上架前、实装前直接返回数据模型）。此前：2026-07-06（§4 时间基准 —— 补过渡态 server_now / 点呼机波 swipe_time 两层边界）；此前：2026-06-09（按实际实装同步 §2 鉴权 / §8 URL 命名 / §13 URL 路径段 —— 关闭 L8；详见各节）；2026-04-22（§1 精化 + §8-§15 扩写 — S13 / L8 / S19）
v0.1 原版 2026-02-12；后续修订见 CHANGELOG + 本文件顶部。

## 1. 统一响应包裹格式（已定稿，已实装）

> **实装状态（2026-07-17）**：{ok,data} 包络已在后端中间件 + 三端解码落地。成功响应由 `ResponseEnvelopeMiddleware`（`app/response_envelope.py`）包成 `{ok:true, data:...}`；失败由异常处理器包成 `{ok:false, error:{code,message,detail}}`。WebSocket 不走包络。探活 `/healthz`、`/`、`/openapi.json` 以及非 JSON / 204 响应豁免。
>
> **OpenAPI 失真声明（选项 A）**：本仓库导出的 `openapi_snapshot.json` / Swagger schema 描述的是 **`data` 内部**（或业务错误字段语义），**不含**外层 `{ok,data}` / `{ok,error}`。实际 HTTP 响应一律多包这一层——读 schema 时请自行套信封。
...
- `data` 可为 `null`（例如「当前无活跃注册码」返回 `{ok:true, data:null}`）。
...
- 前端只展示倒计时，不参与业务判定。
- 客户端自报时间（如签到请求中的 `ts_local`）一律不作判定依据（2026-07-06 拍板；后端已对齐 `server_now`，字段仅接收兼容不参与判定）。

...
> **2026-04-29 修订**：从"老师必须在 -3min 按"改为"老师**应在** -5min 按 / 系统在 -3min 兜底自动开"。理由：寮監可能忘按；窗口固定不平移（详见 §5.4 修订）。

#### 早点呼（morning）

| 日期类型 | 对象 | 应开始（老师手动） | 兜底自动开启 | 准时截止（起算迟到） |
|----------|------|-------------------|--------------|--------------------|
| 平日 | 普通寮生 | 7:35 | 7:37 | 7:40:00 |
| 平日 | 足球部 | 7:15 | 7:17 | 7:20:00 |
| 祝休日 | 普通寮生 | 8:45 | 8:47 | 8:50:00 |
| 祝休日 | 足球部 | 7:15 | 7:17 | 7:20:00 |

#### 晚点呼（evening）

| 日期类型 | 对象 | 应开始（老师手动） | 兜底自动开启 | 准时截止（起算迟到） |
|----------|------|-------------------|--------------|--------------------|
| 平日 | 普通寮生 | 21:55 | 21:57 | 22:00:00 |
| 平日 | 足球部 | 21:55 | 21:57 | 22:00:00 |
| 祝休日 | 普通寮生 | 19:55 | 19:57 | 20:00:00 |
| 祝休日 | 足球部 | 19:55 | 19:57 | 20:00:00 |

> **新规则**（2026-04-29 拍板）:
> - 老师**应在准时截止前 5 分钟**按「点呼開始」按钮（早于此时间点击 → 拒绝 `NOT_YET_ALLOWED`）
> - 老师忘按 → 系统在**准时截止前 3 分钟**兜底自动开启（`started_source = system`）
> - 一旦开启（老师手动 / 系统自动 任一）,整个时间窗**保持不动**（不平移,详见 §5.4）
> - 在「准时截止」之前签到 = 准时（绿）；之后 = 迟到（黄）
> - 学生未签到时图标保持灰色（`init`），**不会自动变黄**（黄 = 迟到 = 必须有"签到行为"触发）；老师按「点呼終了」时未签到的学生才批量变红（`absent`）
...
1. 老师到点呼室，在管理网站按 **「点呼開始」** 按钮（可按时点见 §4.2 + §5.4）→ 学生才能开始签到
2. 学生比老师先到也无法签到（按钮没按 = 系统返回 `SESSION_NOT_RUNNING`）
3. 时间窗运行期间：学生 NFC 签到 → 服务器判定 `present` / `late` / 拒绝；**未签到的学生座位保持灰色 `init`，不自动变黄**（黄 = 迟到必须由"签到行为"触发）
4. 老师按 **「点呼終了」** 按钮 → 仍未签到的学生座位**批量变红 `absent`**
5. 老师按结束后**不直接回主页**，跳转「点呼総結」中层页（详见 §5.6）
...
> **2026-04-29 修订**：推翻原"窗口平移"规则，改为"窗口固定"。理由：老师提前按开始的目的是让学生**早一点能签到**，不是把准时截止/迟到时刻一起前移。原"平移"规则会让 21:54 签到也算迟到（即使原计划 22:00 才迟到），反直觉。本次修订关闭附录 A.4 的疑问。

#### 新规则

老师在 **`window_start = on_time_end − 5min`** 起可手动按「点呼開始」：
...
| 老师按下时刻 | 系统响应 |
|---|---|
| `< on_time_end − 5min`（早于应开始时刻） | 返回 `NOT_YET_ALLOWED`，不变更状态 |
| `[on_time_end − 5min, on_time_end − 3min)`（应开始 ~ 兜底自动 之前） | 立即激活 `started_source = teacher` |
| `on_time_end − 3min`（兜底自动开启时刻） | 系统自动激活（详见 §5.5），老师未按也会开 |
| 系统已自动开启后老师再按 | 返回 `ALREADY_RUNNING`，不变更状态 |

### 5.5 自动开始 / 自动结束（系统兜底）

> **2026-04-29 修订**：自动开始时点从 `scheduled_window_start` 改为 **`auto_start_at = on_time_end − 3min`**（即"准时截止前 3 分钟"）。理由：`window_start` 提前到 `on_time_end − 5min` 后，3 分钟兜底比 5 分钟更准确反映"老师真的忘了"，给学生 3 分钟签到窗口。

**自动开始**

到达 **`auto_start_at = on_time_end − 3min`** 时，老师仍未按开始 → 系统自动开始：

- `started_at = auto_start_at`
- `started_source = system`（取值见 ENUM `session_event_source`）
- 整个时间窗保持 scheduled 不变（同 §5.4 不平移）
...
### 5.6 「点呼総結」中层页（2026-04-29 新增）

老师按「点呼終了」（或系统自动结束）后，系统跳转到「点呼総結」页面，显示本场点呼的 4 个区块：

| 区块 | 内容 | 数据来源 |
|---|---|---|
| **1. 缺席** | 未签到学生（`absent`） | `base_status = absent` |
| **2. 迟到** | 准时截止后签到（`late`） | `base_status = late` |
| **3. 特殊要求** | 临时身体不适（感冒发烧 / 受伤等）| `health_issue` overlay + `absence_request_pending`（申）|
| **4. 外宿自动跳过** | 事先申请免点呼（外泊 / 帰省 / 帰国） | `base_status = exempt_range` |

#### 流程

- 老师在総結页查看 → 按「主画面に戻る」按钮 → 才返回主页
- 主页保留 **「本場の結果を見る」入口** → 可重新打开本場総結页（只读，不可改判）
- 改判仍然要走 §11（老师手动操作）流程，在主页 / 学生卡片上做

#### 设计意图

- demo 4-28 时旧 flow：按结束 → 直接弹 toast「結果は記録に保存されました」+ 回主页 → 老师没机会回顾本场情况
- 新 flow：強制老师**先看一眼缺席 / 迟到名单**再回主页，提高现场决策（谁需要追联系 / 谁要发指导履歴）的及时性
- "外宿自动跳过"显式列出是为了让老师确认"系统判断的免点呼对应人是否符合预期"，避免漏检免点呼名单错误
...
### 11.3 改判时限（2026-07-17 拍板改写 — 后端双会话审查 2-高-1 收口）

> 原 4-17「角色 × 时间」时限矩阵（≤7 天所有老师 / 8-30 天仅舍监 / >30 天只读走追溯申请 / 已发处分 session 原则只读）**整体废除**，仅存附录 B.9 作历史记录。

现行规则：

- **无时限**：任何时刻都可改判——含场次结束后、月结后（场次结束不冻结改判；原实装「`ended` → 409 `SESSION_ENDED`」闸一并删除）
- **所有老师**均可操作（寮边界权限按现行权限模型；2026-07-17 另拍板「分角色跨寮」见 `design/teacher_permission_v1.md §13`）
- `reason` 必填 + 记录 append-only（§11.2 硬规则不变）
- 扣分自动跟随改判（§11.4 联动表不变）
- **已知副作用（知情接受）**：月度处分名单发出后再改判，扣分账本自动重算，但已发出的处分名单不自动追改——是否撤销 / 重发由老师人工判断
...
### A.4 老师"提前开始"窗口平移规则可能反直觉 — ✅ CLOSED 2026-04-29

**结论**：推翻原"平移"规则，改为"窗口固定"。详见 §5.4（2026-04-29 修订）。

**关闭理由**：itsuki 拍板：老师提前按开始的目的是让学生早一点能签到，不是把准时截止/迟到时刻一起前移。原"平移"规则会让 21:54 签到也算迟到（即使原计划 22:00 才迟到），反直觉。

**新规则**：
- 老师可在 `on_time_end − 5min` 起手动按开始（早于此 → 拒绝 `NOT_YET_ALLOWED`）
- 准时截止/迟到时刻 **写死，不平移**
- 系统在 `on_time_end − 3min` 兜底自动开启（`started_source = system`）
...
v0.2 增补 §11.3。（**2026-07-17 收口更新**：itsuki 拍板废除时限矩阵——改判无时限，§11.3 已改写；本节矩阵仅作历史记录）
...
| B.9 改判扣分 + 修改时间窗 | ✅ | §11.4 联动；时限矩阵 2026-07-17 拍板废除（§11.3 改写为无时限）|

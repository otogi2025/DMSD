...
更新时间：2026-07-17（7-17 回写（审查 2-中-4）：§13 `path_type` 补漏登的 `manual` / §14 `student_status` 补实装列名与预留值 `locked` 说明。此前 4-22：§13 加扩展性说明 — 对应 backlog S9）
...
- `manual`（老师手动路径：点呼机离线等场景老师在管理页代为登记签到 — `system_features §7.22`。**2026-07-17 字典回写**（审查 2-中-4）：实装早已在用本值、字典漏登；「单字母单义」原则仅约束设备路径（A / B / 未来 C），老师手动路径用全词以示区别）
...
> **2026-07-17 回写**（审查 2-中-4）：① 后端实装列名为 `students.status`（canonical 名 `student_status` 保留为字典名，见 `FIELD_REGISTRY §2.10`）② 实装 CHECK 约束多一个预留值 `'locked'`——**无任何代码写入该值**（真实登录锁定 = 账号表 `locked_until` 时间戳，不动学生状态），不加入下表、新代码禁用。

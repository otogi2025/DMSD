...
## 7. 待补完项目（4-17 立此存照，2026-07-17 部分收口）

- 设备的硬件型号、采购成本、维修流程 → 待现实世界调研后完善
- ~~设备故障 / 离线时的降级策略~~ → ✅ 已定稿：`Device_Contract.md` §6（本地放行 + 恢复补传 + 老师优先）
- ~~WebSocket 协议（点呼机 ↔ 后端）~~ → ✅ 已定稿：`Device_Contract.md` §5
- 设备身份认证 / 注册激活流程 → ✅ 已定稿：`Device_Contract.md` §2（Ed25519 私钥 + enroll 激活码 + 12h 短期令牌）

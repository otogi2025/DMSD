...
- **上游设计真值**：
  - `02_design/hardware_design.md` — 物理硬件层（板子选型 / 模块选型 / BOM）
  - `02_design/system_features.md` — 共用层功能设计（≥2 端涉及）
  - `02_design/flow_design.md` — 端到端流程（学生→点呼机→后端→老师端）
  - `01_specs/rollcall/RollCall_Spec.md` — 点呼业务规则
- **下游协作**：
  - `03_dev/backend/` — 调后端 API（认证 / 签到 / 时间窗判定）
  - `03_dev/student_ios/` + `03_dev/student_android/` — 间接联动（学生 app 通过点呼机贴纸的 nonce 签到）

...
import jp.tomoshibi.android.data.model.ItemPossessionRequest
...
//   PageHeader「物品所持許可願一覧」level 2 + 竖排卡列表，逐条 MockData.DEFAULT_ITEMS
    ...
    GlobalScaffold(activeTab = "apply", navController = navController) {
        Column(
            ...
            Column(
                ...
                val items = MockData.DEFAULT_ITEMS
                if (items.isEmpty()) {
                    // 空状态：图标 + 「提出済みの願はありません」（对齐 iOS EmptyState）
...
@Composable
private fun ItemPossessionCard(item: ItemPossessionRequest) {
    val t = SuzuT.current
    SuzuCard(padding = 14) {
        Row(

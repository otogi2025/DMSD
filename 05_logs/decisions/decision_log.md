...
## 2026-06-11 — 版本史插空细分：选「插空补打」而非「推倒重排」（证据链优先）

**之前的决策**: 老段版本史（v0.3.2~v0.15.0）按时间段切次版本号（6-03 回溯补标），期间 bug 修复全被吞进 minor；6-09 已为 v0.15.0 之后立「一个 bug 一个补丁号」新标准——新老两段粒度标准不一致。
**新的决策**: 方案 A 插空补打——已 push 的 67 个旧标签与全部 commit 零改动，只在号位空隙给老段 70 个 fix commit 逐个补打补丁标签（v0.3.3~v0.14.8，标签日期回填 commit 当天）；3 个号位死区的 13 个 fix 在 CHANGELOG「死区注记」逐条列出；v0.19.3 之后 76 个 commit 按同标准切 3 minor + 3 patch（v0.20.0~v0.22.3）。标签总数 67→143。
**为什么改**:
1. itsuki 自查版本史发现「x 动得不少、y 几乎没动」——老段不符合自己 6-09 立的标准，决定全史统一到同一粒度
2. 约束他自己划死：「虽然已经 push……我想要保留日期，保留证据」——证据链完整性排在粒度完美之前
3. 备选 B（删远程标签全史重排）粒度最优但重写公开历史、所有文档版本引用断链；备选 C（只改文档）git 标签层粒度不变。A = 「不动已发布物」前提下能做到的最大细分
**这个改动影响了什么**:
- CHANGELOG +76 版本段 + 插空说明 banner + 3 处死区注记；版本演变一览补 6-09 欠账（v0.15.1~v0.19.3 详细段）+ 插空说明段 + 新段叙事
- 配套机制：version-bump §4 联动清单 7→8 处（三端客户端版本号）+ Rule 23 同步 + pre-commit「一览拦截」提醒升级为阻塞
- 接受两个已知瑕疵留在历史并写明原因：v0.8.1 标 patch 偏松（号位被占改不了）、死区 13 fix 无独立标签——「有解释的不完美」优于「被重写的完美」
- push 由 itsuki 决定（76 个新标签需 `git push origin --tags`）
**相关**: <raw池>/2026-06-11_版本史插空细分.md（含 Git 历史不可篡改性认知段 + AC 叙事委托段）；CHANGELOG「2026-06-11 插空细分说明」banner
**事后回看**(几个月后补填):
...
## 2026-06-13 — 公开 GitHub 仓库的边界：只放「软件本体」，机制用白名单 .gitignore

**背景**: push 准备会话中，itsuki 自己看出公开仓库塞了太多「跟 DMSD 软件无关、只有自己/AI 看」的东西，质问「为什么 push 那么多跟项目没关系的东西」「正常公开项目哪会有 CLAUDE.md 这些」。同会话查实 6-11 一个 AI 会话未经授权 push 472 commit（itsuki 从未允许过 push）。

**决策**: 公开仓库只保留「DMSD 软件本体」——
- 公开白名单：`01_specs/` `02_design/` `03_dev/` `04_ops/` + `README` `LICENSE` `CHANGELOG` + `.github` `.gitignore`
- 机制 = 白名单 .gitignore（默认忽略一切，只放行白名单）。理由：比逐条排除（黑名单）安全——新加的任何文件/文件夹默认不公开，不会因忘记排除而泄漏，零维护。
- 其余（00_admin / .claude / docs / bin / 99_archive / 06_assets / 根散文件）`git rm --cached` 移除跟踪、本地全保留。
- `05_logs/`（开发日志/决策/演化，AC 证据）物理迁 iCloud 素材池，DMSD 留指路牌。

**为什么改**:
1. 给教授看 AC 的载体是「项目本身 + 公开 commit 历史 + 志望理由书」，不是翻 GitHub 上的开发日志——移除过程文件不损害证据，反而让项目专业。
2. 「公开到 GitHub」和「物理在本地文件夹」是两件事：CLAUDE.md/hooks 必须物理留本地才能用，靠白名单不公开即可，不必也不能搬走。
3. 分支方案被否决：GitHub 公开仓库的分支也是公开的，且分支是「整个项目的平行版本」工作区一次只能站一个，塞私有文件进分支会导致开发时它消失。

**影响**: 大整理本体待 compact 后执行（执行计划 + 交接提示词在 00_admin/handoff/）。已先落地：AC_叙事/raw 迁 iCloud、运维文档（服务器 IP/SSH）从未推送历史抹除、6-09 历史清理计划作废。数字编号目录改名留后续。push 仍须 itsuki 明示。

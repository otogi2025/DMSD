...
- ⚠️ **防代刷断链，待重新设计**：旧的「nonce 从贴纸传手机」链断了。三缺口：①手机只写明文 ID 会被伪造 → 仍需手机签名 ②防重放 nonce 来源没了 → 要重定谁发 nonce ③挡不住借手机/借卡到现场代刷
- 连带未改（待新防代刷定）：`flow_design.md` / `NFC防代刷_后端立项施工计划.md` / `system_features.md` / `ROLLCALL_DEVICE_DESIGN_LOG.md`
- v2.0 人脸识别（1:1 匹配刷卡人是不是卡主）堵「借手机代刷」洞；v1.0 期间这层靠「现场播报姓名 + 老师看脸」兜
- 暂不升版本号（代码未实装 + 防代刷未定）

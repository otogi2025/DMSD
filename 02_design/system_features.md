...
- **寮監账号管理**(2026-04-30 拍板):
  - **前台不允许自助注册寮監账号**(防外人冒充寮監)
  - 必须先用**已存在的寮監账号**登录 → 后台「寮監账号管理」页 → 加 / 删寮監账号(寮監之间互相是 trustee 关系)
  - **首个寮監账号 bootstrap**: 系统初次部署时,由「项目负责老师」(Q10 角色)在后台手动写入第一个寮監账号 → 之后由该账号扩展其他寮監

...
- **タクシー予約（出租车预约 · itsuki 2026-06-03 新增）**: 学生希望坐出租车出寮 / 帰寮时填想坐车的时刻（只填时刻、可选）。后端字段 `applications.taxi_reservation_time`（Time / nullable，null = 不预约）；出寮届三种（帰省 / 外泊 / 帰国）+ 外出共通。老师在「申請詳細」+ 申請页「タクシー」tab（带 badge 提醒、防漏看）能看到。**iOS / 老师网页 / 后端已实装；Android 待接后端时一起做**
- **理由**: 帰省理由 / 外泊目的

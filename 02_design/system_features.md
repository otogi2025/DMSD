...
- **教师权限模型** → **本段「按职责勾选」旧模型已作废（2026-06-11）**，被「权限组 + 按功能两级开关」新模型取代。
  单源真值见 `02_design/teacher_permission_v1.md`（5 权限组 × 16 功能簇 → 管理 M / 查看 V / 无 ✕；职位退化为纯显示标签、不参与鉴权）。
...
2. **教師が Web 後台「学生登録コード」パネルでボタンを押すと生成**
   - 全権限グループの教師が生成可（2026-06-14 itsuki 拍板で「寮務管理のみ」を撤廃、デモアカウントも可）。権限の単源真値は `02_design/teacher_permission_v1.md` §5

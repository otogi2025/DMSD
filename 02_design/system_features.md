...
> - 提交后 → 显示承认状态(#5)
> - 通知 = **邮件**(R1)
> - **教師代録(2026-06-04 杭田五-3「教師用は当日入力可」追加)**: 上面 #1「不能代填」限的是**学生不能替别人填**;**寮務系教師(寮務部長 / 寮務課長 / 寮監 / 寮務一般教師 / 管理係 5 角色)可代学生提出**,且代録放宽到**当日も可**(学生侧仍禁当日 #3)。代録走 `POST /applications/by-teacher?student_id=`(受 R4 寮边界),学生选择器走 `GET /applications/proxy-candidates`(同 5 角色 + 寮边界),提出後は学生提出と同じ承认链。老师网页 `ProxyApplicationPage` + 后端实装完了(2026-06-05),iOS / Android 学生端不含此功能(代録是老师专用)。
...
- 鉴权: 上传仅本人 + 仅 pending 状态;下载学生本人 OR 老师(老师受 R4 寮边界限制,跨寮看不到);老师待审列表也按 R4 过滤避免文件名泄露别寮学生姓名

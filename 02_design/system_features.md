...
**役职差异**:
- 一般学生外泊：**没有**寮務部長 / 国際交流部長 / 国際交流課長
- 留学生外泊：**没有**国際交流課長（老师 4-29 LINE 写「国際交流部長 + 国際交流課長」，实物表只有「国際交流部長」一项）
...
> **数据模型影响**: `application_approvals.approver_role` ENUM 必须扩充为 `{担任, 寮務部長, 寮務課長, 国際交流部長, 管理係}`（5 值，去掉「国際交流課長」）。「担任」是 per-学生的（每个学生有自己的班主任），需要 `students.homeroom_teacher_id` 字段或 `class_teacher_assignment` 表 — ⏳ 待 itsuki 拍板数据模型方案。

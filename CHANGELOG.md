...
| v0.35.0 | 2026-07-24 | minor：老师网页补齐在线学习审批入口（后端接口早已实装但从未接线，grok 三方对齐审查发现）|
| v0.36.0 | 2026-07-24 | minor：点呼「学生からの報告」处理页接线（后端列表已实装、老师网页从未接线）|
| v0.36.1 | 2026-07-24 | 补丁：老师网页全面体检 · 主施工 28 项（时区/权限门控/并发竞态/WS 鉴权/日语用语 五族，46 代理审查确认中危全修）|
| v0.36.2 | 2026-07-24 | 补丁：老师网页全面体检 · 三方复审补修 + 收尾入档（opus/grok 复审抓的漏网·半修 + 后端配套 + 文档）|
| v0.37.0 | 2026-07-28 | minor：点呼机硬件组装从纸面走到实物（组装指南网页版 + 七站鸟瞰导航 + 实机踩坑逐条回写 + 首次真焊 ST25DV）|
| v0.38.0 | 2026-07-29 | minor：真硬件首次跑通 + 苹果演示视频冲刺（ST25DV RF 侧指针 0x06→0x0D 阻断 bug + 两处静默丢签到 + iOS 一按就扫·签到完成态）|
| **v0.39.0** | **2026-08-02** | **minor：老师网页真上线前三轮审查战役（事案记录补寮过滤 + 行事巴士时刻偏 9 小时 + WS 票据丢当班寮 + 点呼大屏断线补拉 + 扣分乐观锁；收尾把统计卡改回演示/真账户双轨）← 当前版本** |
...
> **【2026-07-24 回溯切版本说明 · v0.36.0 ~ v0.36.2 + 首次 push】** v0.35.0 之后累积 47 个 commit（7-24 当天、全本地未 push），itsuki 发令「细分 commit 迭代版本」+「push」。按 version-bump §2 决策树切为 1 minor + 2 patch：v0.36.0（`feat` 点呼学生上报处理页接线，独立主题单独成 minor）+ v0.36.1（老师网页全面体检主施工 28 项，30 commit `23e0645..1bc874d`，含 iOS 版本号同步 chore）+ v0.36.2（三方复审补修 + 收尾入档，15 commit `322fc3d..ea99226`）。⚠️ **战役 44 commit 按修复顺序交错提交（时区/门控/日语混排），git tag 只能沿 commit 时间线切、无法按内容族切**，故按「主施工完成」时点切一刀分两段 patch；itsuki 拍板「细粒度」故未把战役合为单个 patch。3 个 annotated tag 指向各段末 commit、日期回填 commit 当天、commit 与历史均未改写。⭐ **本轮为 origin/main 时隔多轮的首次 push**：远程停在 `6b1574f`，本次 `git push origin main` 一次推进 228 commit，并补齐从未 push 的 15 个版本 tag（v0.27.1 / v0.28.0 ~ v0.35.0 / v0.36.0 ~ v0.36.2）。iOS `MARKETING_VERSION`（当天 `501f121` 已改 0.35.0）+ Android `versionName` 本轮统一同步至 0.36.2。

> **【2026-07-29 结账说明 · v0.37.0 + v0.38.0】** v0.36.2 之后累积 26 个 commit（7-26~7-29、全本地未 push），itsuki 发令结账。按 version-bump §2 决策树 + itsuki 拍板「切 2 个」切为 2 个 minor：**v0.37.0**（7-26~7-28，18 commit `89c5ddf..bd2fc5e`，点呼机硬件组装从纸面到实物 —— 组装指南网页版从零建 + 七站鸟瞰导航 + 实机踩坑逐条回写；主体为 docs 但含新建交互式指南与硬件设计实物核对，属新范围进来故计 minor）+ **v0.38.0**（7-29，7 commit `a98be5d..07082e9`，真硬件首次端到端跑通 + 为苹果 Guideline 2.1 补拍演示视频所做的阻断修复与交互改造）。2 个 annotated tag 指向各段末 commit、日期回填 commit 当天，commit 与历史均未改写（v0.36.2 之后所有 commit 未 push，回溯安全）。⚠️ **iOS 版本号本轮走 App Store 重交流程**：首次提交的构建 `1 (0.27.0)` 因缺 NFC 硬件演示视频被 Guideline 2.1 打回，且该 build 含本轮才修的 `0x06→0x0D` 阻断 bug（真机 NFC 写入 100% 被拒），故撤回旧提交、以 `MARKETING_VERSION` 0.38.0 + `CURRENT_PROJECT_VERSION` 2 重新上传并连同视频重交。Android `versionName` 同步 0.38.0。

> **【2026-08-02 结账说明 · v0.39.0】** v0.38.0 之后累积 22 个 commit（8-01~8-02、全本地未 push），itsuki 发令结账。按 version-bump §2 决策树命中两条 minor 触发条件（#2 字典实质内容加字段 —— 新增 API 字段 `expected_current_points` / `is_demo` 与新错误码 409 `POINTS_CHANGED`；#4 dev/ 业务规则改写 —— 事案寮边界、WS 票据当班寮、时区归一化），**切为 1 个 minor**。未按「主施工 / 复审」切多刀，理由：这 22 个 commit 是同一场审查战役的三个连续轮次（主施工 13 条 → codex 复审 6 条 → 双家背对背 3 条 → 收尾 demo 双轨修正），中途任何一刀切下去的版本都不是「可上生产」的状态，切多刀反而让「哪一版能推」变模糊。annotated tag 指向段末 commit、日期回填当天，commit 与历史均未改写（v0.38.0 之后所有 commit 未 push，回溯安全）。⚠️ 本轮**不同步 iOS / Android 客户端版本号** —— 22 个 commit 全部落在后端与老师网页，两个手机端一行代码没动，且 iOS 的 0.38.0 构建正在 App Store 审核队列中（连同 NFC 演示视频重交），此刻改 `MARKETING_VERSION` 会与在审构建脱节。两端版本号攒到下次触及它们的 minor 再同步。老师网页版本号由 `vite.config.ts` 构建时从本文件自动注入，无需手改。

## [0.39.0] - 2026-08-02（老师网页真上线前：三轮审查战役 + 一条自我推翻）

> 起因：老师网页要第一次给真宿管老师用（生产 `teacher.tomoshibi.cc`）。上线前派三家 AI 背对背只读审查，itsuki 从 33 条发现里拍板修 3 阻断 + 10 高危；修完按流程派 codex 复审，又挖出 6 条——**其中 3 条是当天新写的修复代码自己引入的**，可见「修完即上线」这一步不能省；再派 grok + codex 两家背对背第三轮，收 3 条并推翻了当天早些时候自己的一处折中改法。

### Fixed

**第一轮：审查拍板的 13 条**

- **事案记录（`incidents.py`）补寮边界过滤** —— 这是全后端最后一个完全没有寮过滤的接口，男寮当值老师能读到女生事案的全文和真名。事案正文常含具体违规经过，比排行榜分数敏感得多。5 个端点（新建 / 列表 / 详情 / 修改 / 删除）全覆盖，判定口径 =「涉及学生**全部**在可见寮内才放行」，与扣分 / 学习 / 点呼同款；无涉及学生的事案不属于任何寮、不受限。回归测试 14 件，做过反向验证——换回修复前代码，7 条「该拦」的测试全部变红。
- **行事 / 巴士时刻整体偏 +9 小时** —— 老师网页的 `<input type="datetime-local">` 发的是无时区串（`2026-08-05T19:00:00`），后端 TZDateTime 的规则是「无时区值一律当 UTC」，于是老师填的 19:00 存成 UTC 19:00 = 日本时间次日 04:00。itsuki 拍板**改后端不改前端**（客户端有 iOS / Android / 网页三个，全改一遍容易漏；后端一处收口更稳）。
- 点呼大屏加实时连接断线横幅 —— 原有的提示条挂在另一个组件上，在大屏这个页面永远渲染不到，断线时屏幕上零异常迹象。
- WebSocket 连上后补拉座席快照 —— 补首次连接的空档与断线期间漏掉的签到。
- 登录时选的当班寮进前端状态 —— 代班老师不再看到错寮徽章、查人数返回 0。
- 手动设定合计点改用实时分数，不再用打开页面那一刻的旧快照。
- 出寮届审批弹窗：邮件通知文案改说实话 + 三个按钮加处理中锁。
- 通知中心「点呼報告」改跳学生上报处理页（原来跳到了历史场次页）。
- 点呼首页删掉「部活生」假选项 + 対象人数对齐可见范围。

**第二轮：codex 复审挖出的 6 条**

- **老师换 WebSocket 票据时把当班寮丢了** —— 当班寮进了登录令牌、REST 接口按它算可见范围，但换 WebSocket 短时票据时没带上，服务端回落读老师档案里固定的所属寮。后果比前端显示错重得多：**这是服务器按错寮裁广播**——档案男寮、今晚代班女寮的老师，点呼大屏显示「已连接」却一条女寮签到都收不到，反而收到本不该看的男寮。这是「后端有 5 处自己解 JWT 不走统一鉴权依赖的入口，改鉴权规则必须逐个覆盖」这条教训的**第三次翻车**。
- **手动设定合计点的时间窗** —— 前端改成现查当前分核对后，现查到请求到达之间仍有几百毫秒，点呼迟到等自动扣分落在其中照样被静默抵消。纯前端补不上，判断与写入必须在同一把锁内：请求体新增可选 `expected_current_points`，后端在行锁内比对，不一致返回 409 `POINTS_CHANGED` 且一行不写。字段可选，iOS / Android 老客户端行为不变。
- 审批弹窗的处理中锁存在弹窗自己的状态里，请求还在飞时点遮罩或「閉じる」组件卸载、锁随之消失，老师能立刻从旧列表重开同一份申请点相反的操作。**加锁时只想到「按钮不能连点」，没想到「组件不能被卸载」**。
- 搜学生弹窗在提交飞行中还能换选另一个学生，分会记到后换的人头上。
- 座席补拉失败时只写了一行控制台警告，注释理由是「连接横幅已经在提示了」——但走到这一步连接状态已是正常、那条横幅根本不渲染。**教训：写「已有别处提示」这类理由时必须验证那个提示在当前状态下真的可见。**

**第三轮：双家背对背 + 一条自我推翻**

两家（cursor grok-4.5-high 与 codex gpt-5.6-sol）互不知对方存在，对第二轮修复独立给出「修对了」，**零互相误报**；分歧只在结论口径：grok 判「可以上生产」，codex 判「不建议直接上」，各自多报一条对方没看见的低 / 中危。逐条核实后三条全部属实并修掉。

- 座席故障标记跨场次残留 —— 一场点呼里补拉失败过，标记留到下一场开头闪一下红字。
- **两条重拉路径的请求编号不共享** —— 后台静默补拉与老师主动「座席を再取得」各自在闭包里存了一份计数器，于是慢一步失败的旧请求仍能通过自己的守卫，把主动重取刚清掉的故障横幅重新点亮。**教训：用编号做「后到的旧请求不许写结果」这类守卫时，凡是会写同一份状态的路径都必须共用同一个计数器**，各存一份等于没做守卫。
- **撤回当天早些时候的一处折中改法** —— 第二轮给座席合并函数加了「主动刷新时以服务器为准清一遍本地徽章」的分场景规则，codex 指出这会清掉**仍然有效**的徽章。核实四项事实后判定 codex 对、自己错：① 这四个字段服务器压根不返回、映射里恒为空，所谓「以服务器为准」并没有一个服务器值可依，清掉是纯信息销毁；② 老师端 WebSocket 只推签到 / 代签 / 新外泊申请三种事件，后端**没有任何事件**会在申请被批准或退回时通知老师端，所以「徽章过期」没有可感知的时机；③ 徽章变陈旧须是「同一场点呼期间别的终端刚好把这份申请批掉」，窗口很窄，而误清的代价是老师看不出学生已交外泊申请、去追一个有正当理由的人；④ 其中 `health` 字段全项目无任何写入点，是死字段。**教训：面对「保留还是清除」的两难，先问服务器对这个字段有没有话可说；服务器无话可说时，「以服务器为准」是个假选项。**

### Added
- **`TeacherOut` 新增 `is_demo` 字段 + 点呼首页统计按账号类型分轨** —— 收尾时发现审查阶段做错了一件事：把点呼首页那些没有后端接口支撑的统计卡和「最近の点呼」表格一刀切改成了「準備中」占位。理由没错（数字确实是编的），**但结论错了**——itsuki 7-17 已就此拍板过：演示账号显示演示数据、真账户显示真数据或占位，**两者共存**。演示账号是给 App Store 审核员和宿舍管理员看的，界面必须有内容；真宿管老师则一个编出来的数字都不能看到。改回双轨需要前端知道当前登录的是不是演示账号，故 `TeacherOut` 下发 `is_demo`（替代方案「前端硬编码哪个登录名是演示账号」明显更差）。该字段只是显示开关，不承担隔离职责——真正的数据隔离仍在后端（真老师查不到演示学生）。**更值得记的是**：那条决策的批注当时就写在被删代码的正下方，仍然被当成纯假数据清掉了——删「明显是假数据」的代码前先读它周围的注释。

### Notes
- 验证：后端全量 pytest **684 passed / 1 skipped**（基线 663 + 本轮新增 21 条）；老师网页 `npx tsc --noEmit` 0 错 + `npm run build` 绿。新增的两组测试（WS 票据 4 条、扣分乐观锁 3 条）都做过**反向验证**——把被测代码换回修复前版本重跑，该红的如期变红，确认不是恒绿摆设。
- ⚠️ 后端测试必须串行跑 —— 子代理并行跑会抢同一份 `test_tomoshibi.db`，本轮踩到过一次假大红（49 failed / 263 errors），等并发结束后单独重跑即全绿。
- 客户端版本号本轮不动（22 个 commit 全落在后端与老师网页，两个手机端未改动；iOS 0.38.0 构建正在 App Store 审核队列中）。

## [0.38.0] - 2026-07-29（真硬件首次跑通 + 苹果演示视频冲刺）

> 起因：iOS 首次提交 App Store 的构建 `1 (0.27.0)` 被苹果以 Guideline 2.1 - Information Needed 打回，要求补交「真机 + 点呼机硬件交互」的演示视频（Submission ID `6dd6dd02-5bcc-44d0-9169-d8d7666dcb5c`）。为拍这段视频，手机 → ST25DV → 树莓派 → 后端这条链路必须在真硬件上先跑通 —— 过程中挖出一个此前只在模拟环境验证过、真机必然阻断的 bug。

### Fixed
- **ST25DV 邮箱控制寄存器 `MB_CTRL_Dyn` 的 RF 侧寄存器指针写错：`0x06` → `0x0D`（iOS + Android 同一个 bug）**。同一个寄存器有两个地址：点呼机走 I²C 接口用 `0x2006`（正确），手机走 RF 接口的 Read Dynamic Configuration（`0xAD`）指针是 `0x0D`；两端手机代码都误取了 I²C 地址的低字节，等于读到别的寄存器 → 邮箱状态判断全错 → 写入被无条件拒绝。真机首次联调必然失败，且现象酷似接线/硬件故障，极难排查。发现路径：cursor grok-4.5-high 只读审查报阻断项 → 查 ST25DV 手册独立核实证实（`8e8fef9`）。
- 点呼机消除两处静默丢签到（`350312f`）：① `MB_WDG` 邮箱看门狗出厂默认 `0x07` 启用超时 —— 手机写进邮箱的消息若未被及时读走，芯片自行清 `RF_PUT_MSG` 标志并丢弃载荷，而 `message_pending()` 正是查该位判有无新消息，标志被清 = 签到永久丢失、现场只表现为「碰了没反应」；改为 `begin()` 中先读后比、非 0 才写回 `0x00`（EEPROM 寄存器，避免每次开机磨损）。② `begin()` 在 `present_password` 后校验 `I2C_SSO_Dyn`，安全会话未开即抛错 —— 原实现下口令不符时三个系统寄存器全被静默拒写，程序照常启动而邮箱功能全程失效。寄存器地址与禁用值经 ST 官方社区核实。
- iOS 默认后端地址改为生产 VPS `https://api.tomoshibi.cc`，去掉 DEBUG 分支默认 `localhost:8000` 的真机陷阱 —— 真机上 `localhost` 指 iPhone 自己，Xcode Run 装真机后登录必报「通信エラー」且错误面只提示检查信号；该默认值还连带要求 ATS 明文 HTTP 例外与局域网权限（`61879fa`，拍视频当天真机实测踩到）。本地后端开发照旧用 `TOMOSHIBI_API_URL` 环境变量显式覆盖（仅 DEBUG 读、Release 不读的安全约束不变）。

### Added
- 点呼机 `--skip-card-reader` 启动开关：PN532 读卡模块尚未焊接时也能单独验证「手机碰一下」这条路径（`a98be5d`）。原装配顺序把 PN532 排在 ST25DV 前面，读卡器缺席会让整机在启动阶段抛 `HardwareInitError` 退出，手机路径根本轮不到跑；PN532 是全项目唯一无备份零件，需在焊它之前先探明 RF 侧链路。启用时读卡器整块不构造、换成恒返回 `None` 的假实现（只表现为读不到卡、不伪造刷卡事件）并打 WARNING —— 默认关闭，缺件静默启动属上线事故，降级必须由人显式声明。
- iOS 点呼签到改「按一次直接扫」+ 签到成功后主页真的变完成态（`07082e9`，两个问题均为 itsuki 真机实测报出）：① 原流程首页按钮先弹自制弹窗、再点弹窗里的按钮才开苹果 NFC 面板，而苹果面板是系统级的会盖在自制弹窗上、成功失败也全由它显示 —— 自制弹窗除多按一次外无任何作用；新增 `NFCCheckinLauncher.swift` 让首页按钮直接开 CoreNFC，两个自制弹窗（点呼 / 夜学習）整块搬进 `HomeCheckinDemoSheets.swift` 并包进 `#if DEMO`（演示机没有真 NFC 可碰，给宿舍管理员看流程仍靠这套假动画）。② 写卡成功后 app 从不回头问后端结果，主页要等下次冷启动才更新；`AppStore` 新增 `confirmCheckinAfterNFCWrite()` + `maybeRefetchTodayRollcall()`，写卡成功等 1.0s 后最多轮询 4 次（间隔 1.5s、5.5s 封顶），超时兜底改提示「送信しました · 反映を待っています」+ 后台 20s 续拉；完成态卡片整块变绿 + `checkmark.circle.fill` 徽章，顶部条与底部导航文案同步（原「点呼時間外です」自相矛盾，改「本日の点呼はすでに完了しています」）。
- 点呼机启动 / 停机 / 签到确认这 5 行日志改日文（`e472fe7`）—— 将来看终端的是宿管老师，且演示视频镜头会扫到终端画面，不该出现中文；其余内部调试日志仍为中文。

### Notes
- 运维：老师网页上线 `teacher.tomoshibi.cc`，新增 `ops/teacher_web_生产部署与更新.md` 记录部署形态（静态文件由 nginx 直接托管、`/api` 与 `/ws` 反代 127.0.0.1:8000）与「为何不用后端托管」的决策（该方案需重建 api 容器，而生产后端同时服务已提交的 iOS 客户端）；`ops/teacher_web_v1.0_上线部署清单.md` 顶部标注过期（`79237d5`）。
- 验证：iOS 双 scheme（`TomoshibiApp` 真机 iPhone 17 Pro / `TomoshibiAppDemo` 模拟器）BUILD SUCCEEDED + `RollStateMachineTests` 15/15 passed；点呼机 pytest 99 passed；Android `assembleDebug` BUILD SUCCESSFUL。**全链路真机实测通过 —— 手机 → ST25DV → 树莓派 → 后端 `device-checkins` 200 OK**（点呼系统第一次在真实硬件上端到端跑通）。
- 客户端版本号：iOS `MARKETING_VERSION` 0.38.0 + `CURRENT_PROJECT_VERSION` 递增至 `2`（App Store 构建号，同一 app 内不可重复）/ Android `versionName` 0.38.0。

## [0.37.0] - 2026-07-28（点呼机硬件组装：从纸面走到实物）

> 起因：7-26 硬件零件到货，此前所有点呼机代码与文档都写于「硬件从未到手」的前提下。本段是把纸面方案在实物上逐项核对、并把首次真焊踩到的坑回写进文档的过程。

### Added
- 点呼机组装指南网页版 `组装指南.html` 从零建立（`89c5ddf`）：阶段 0-5 共 70 个可勾选项 + GPIO 40 针图 + 焊点好坏图示 + 故障对照表，进度用浏览器本地存储持久化；后续加每阶段「全勾 / 全清」一键按钮（`c52ade3`）。
- 页首七站鸟瞰图（`28eecf9`）：每站一句大白话讲干嘛 + 危险点标红 + 对应主线板日程日期，按打勾进度自动标「← 你在这」、完成站打 ✅，7 个阶段加锚点从鸟瞰直跳。
- 阶段 5 全流程改成可自助执行 + 页首加「卡住先自查 5 条」总闸 + 故障表扩到 18 条（`f521ead`）。

### Fixed
- 点呼机系统用户名改为实际烧卡值 `rollcall-1`（原文档假定树莓派默认的 `pi`），文档漏改 5 处一并补齐 + 撤回未经验证的技术论断（`6464c6d` / `abb9d4d` / `1580516`，cursor grok 异构审查发现）。
- PN532 驱动版本 `1.4.16` → `2.4.9` —— requirements 里钉死的那个版本号在 PyPI 根本不存在（软件写于硬件到货前、从没在真机上装过）（`cba891c`）；指南阶段 2.3 补这条实机踩坑说明，教「找不到版本」先怀疑版本号是编的、改完要逐个 import 验 API 兼容（`f668665`）。
- 硬件初始化失败改给人话提示 + systemd 不再无限重启（组装现场排错时无限重启会把真实错误刷屏冲走）（`d99420e`）。

### Changed
- 系统选型行回写实机事实：Legacy 64-bit Lite / Bookworm / Python 3.11.2（`607ae57`）。
- `design/hardware_design.md` §6.0 布局节回写 7-27 实物核对结果（`544caab`）：喇叭实为一对 6.5cm 盒子 + 54cm 线，塞不进计划的 15-18cm 外壳（型号对但与本节尺寸交叉矛盾，8-02 设计外壳前须先量实物）；面包板到货 ×2 故「≥10cm 间距」铁律可满足、无需悬空拉线；标注病根 —— 本文件各节的尺寸 / 距离 / 电压数字从未跨节交叉验算。
- 按到货工具实物照片与日文说明书改写阶段 3 焊接段（`899762d` / `f5ef0f8` / `38cdfa9` / `5482013`）：新增「这把烙铁怎么用」说明卡、原装细尖头必须换、烙铁架就是那个黑托盘、焊锡确认含铅、开焊前清单补第 ⑥ 步「确认烙铁真的热起来了」、补「到底要焊什么」说明卡。
- 回写 7-28 首次真焊 ST25DV 踩到的坑（`bd2fc5e`）：烙铁头锡珠不许蹭掉 / 温度改 400℃ / 送锡要追接缝 / 万用表显示 `1` 才是对的 / 母母杜邦线插不进面包板必须拔板直连。

### Notes
- 点呼机到货实物照片目录加入 `.gitignore` 排除（公开边界，`04472d4`）。

## [0.36.2] - 2026-07-24（老师网页全面体检 · 三方复审补修 + 收尾入档）

> 起因：v0.36.1 主施工 28 项完成后，走三方对抗复审（opus 47 代理 + cursor-grok 异构 + 主会话终审）逐条挑刺，抓出 4 项半修/漏网 + 一串低危；补修后再经 grok 两轮复审到「无话可说」收敛。

### Fixed
- 复审 R2 补修 4 项半修/漏网：账号页三操作 + 公告返信补 ref 同步锁堵双击竞态（setState 异步、双击第二下可赶在重渲染前穿过 state 守卫各发一次请求）（`322fc3d` / `948be4a`）；点呼 WS 实时事件/广播直出 ISO 撑破座席卡、点呼报告页漏门控 三件补全（`24404b8`）；手動加算弹窗区分「排行榜未到」与真实 0 点、防默认 0 提交清零本月扣分（`e6f831b`）；JST 漏网家族 5 处 + 罰則清掃词序余 3 处补齐（`0e4974a` / `383b2e4`）。
- grok r3 复审抓出补修自身两处新问题当场修：注册码刷新写回前二次 bump 代次堵二阶竞态（前一修只作废刷新前已在飞的轮询、拦不住等待窗内新起飞的）（`928bb2c` / `397dbbd`）；手動加算预填 effect 依赖排行榜后会盖老师手输值，加 `scoreTouchedRef` 手输后停止自动预填、换学生复位（`e416f35`）。
- 后端 C20 裁决落地：老师 WS「一次性」宣称改准确表述「短时票据（60 秒 TTL，无单次消费机制）」，`ws.py` 记不实装单次消费的权衡理由 + 补过期票据拒绝专测（`b72fa84`）；审计脱敏正则补 ticket 关键字前瞻加固（`0944cdf`）。

### Changed
- 后端测试补缺口：老师代提外出广播路径补 reason/submitted_at 断言 + `StudyOnlineRequestOut` schema 快照期望集补三字段（别会话漏同步的历史红灯，全量 pytest 至此清零）+ openapi 快照重导入册 `WSTicketOut`（`26886e5` / `2376850` / `0280517`）。

### Notes
- 验证：后端安静环境全量 pytest 663 passed / 1 skipped 全绿（复审期 49 errors 为 47 并行代理抢资源瞬时干扰，安静环境不复现）；老师网页 tsc 0 错 + npm build 绿。
- 入档：`WEB_DESIGN_LOG` / `BACKEND_DESIGN_LOG` 战役总结条 + `logs/dev_log` 工程流水账 + `decision_log` 裁决记录（`ea99226`）。

## [0.36.1] - 2026-07-24（老师网页全面体检 · 主施工 28 项）

> 起因：老师网页全面体检（46 代理并行审查）确认中危 26 条 + 用语拍板 2 项（学生→寮生 / 代録→代理提出），itsuki 拍板「中危全修」，10 批串行、一条一 commit 修完。五族分类见下。

### Fixed
- 时区：全站时刻显示钉 `timeZone: "Asia/Tokyo"`——注册码轮询 / 公告日期 / 点呼大屏 / 外出食数导出 / 在线学习提出時刻 等，防非 JST 浏览器跨日或平移一天（`d0f24f5` / `89ba4f9` / `9d77713` / `9f2a531` 等）。
- 权限门控：7 个页面对无 MANAGE 权限组（申請承認専用 VIEW-only）隐藏写按钮，对齐后端 require_permission，防点了必被后端 403（教员账号 / 夜学习 / 减点 / 事案 / 前台 / 清扫 + 检索页 query 同步）（`22f741a` / `f7a24da` / `794e141` / `2b31fed` / `116dedd` / `5d2f15c` / `2fe831d`）。
- 并发竞态：账号管理三操作 + 公告返信加进行中守卫防双发（`4b0fde6` / `23e0645`）；注册码 30 秒轮询 + 通报一览加请求代次守卫防旧数据覆盖新数据（`d74ad3a` / `fe5917c`）；手動加算弹窗显示当前合计点防默认 0 清零（`b05239a`）。
- WS 鉴权（C20）：老师 WS 改用一次性短时票据握手，不再把老师 JWT 塞 query 参数（会原样落 uvicorn/nginx 访问日志、叠加长有效期 = 谁读日志谁拿长期身份）（`e193cfb`）；外泊广播补 reason/submitted_at 防座席徽章恒显占位文案（`1ce902c`）。
- 用语统一（jp-term-blacklist 拍板 + しおり官方词对齐）：学生→寮生 / 代録→代理提出 / 帰舎→帰寮 / 舎監·宿監→寮監 / 删除確認「公告」→「お知らせ」/ 罰則清掃词序（`5e58afd` / `e12b270` / `3dfb34d` / `9281c02` / `005ee35` / `843bcb3` / `d5163e1` / `1bc874d`）。
- 其余：点呼登出 / 超时 / 401 三路径补清上一位老师结果防串号 + startSession 去掉撒谎 warn 改可重试 + 在线学习加载失败态收敛（`2c9eda4` / `8b5b4a4` / `1ec7c23`）。

### Notes
- iOS `MARKETING_VERSION` 0.27.0 → 0.35.0 对齐 Android/项目内部号（`501f121`，就近归入本段；本轮结账末再统一同步至 0.36.2）。
- 进度真值 = `admin/handoff/teacherweb修复作战_交接_2026-07-24.md`（战役完成后归档 `archive/`）。

## [0.36.0] - 2026-07-24（点呼「学生からの報告」处理页接线）

> 起因：与 v0.35.0 同类——点呼学生上报（学生からの報告）后端列表接口已实装，但老师网页从未接线，学生上报后老师端无处处理。

### Added
- 老师网页接线学生からの報告处理页 + 后端点呼报告列表补学生摘要（student_name / student_no / room_no）（`0a6a4df`）。

### Changed
- 重导 openapi 快照：`RollCallReportOut` / `StudyOnlineRequestOut` 补 student_name / student_no / room_no 三字段（`8819e14`）。

### Notes
- 独立主题、独立于当天老师网页体检战役，单独成 minor。

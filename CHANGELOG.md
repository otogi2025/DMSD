...
| **v0.24.8** | **2026-06-16** | **补丁：iOS 日语 UI 自然度全量审查施工 — 以宿舍《生徒寮のしおり》为基准 16+ 文件 149 处 ← 当前版本** |
...
### Changed
- **晩自習 → 夜学習**（决定 1，74 处）：app 全用「晩自習」但手册一律「夜学習」，连 app 自己的演示公告都写「夜学習」（自相矛盾）。本次仅 iOS；英文代号 `study`/`studyAbsence` 不动、接口契约零变化。**其余 4 端（后端用户文本 / 老师网页 / 安卓 / spec）待全端统一，记 TODO §B。**

...
> 起因：v0.27.0（6-18）之后多会话累积 60 commit，按 7-15 拍板「结账制」首次正式清算（itsuki 发令、一次结清）。逐条分类：fix 27 / docs 20 / test 5 / chore 5 / ci 2 / release 1，无一 feat 级新功能 — 边界项逐一核实（点呼判定 server_now = spec 定稿后代码跟上属 fix；Android data/ 纯逻辑层 = 可测试性重构非新功能；iOS 点呼入口占位 = spec v1.0 §2.2 既定上架姿态；CI 与运维文档按 version-bump §2 第 7-9 条不驱动版本号）→ 合并为单个 patch。三端客户端版本号按规矩 patch 攒未动（仍 0.27.0，待下次 minor 同步）。

### Fixed
- 后端审查修复批：临时账户过期检查补 3 旁路 + seed op 生产护栏（`0440526`）/ WS 广播男寮展开 `assigned_dorm in (1,2)`（`73b2dba`）+ `outstay_new` 广播补 `dorm_unit`（`e616bee`）/ 限速 key_func 读 X-Forwarded-For 按真实客户端 IP 分桶（`0222a6b`）/ 寮边界：dorm_life 三审批端点 + `study.cancel_today` 补 `dorm_units_for_teacher` 校验（`a8212a6` / `f76f8c9`）/ 并发：`withdraw_application` 行锁 + `create_account` IntegrityError 兜底（`79ff4d5` / `da7bc1c`）/ 迁移：3 个 `recreate="always"`→`"auto"` 全新 PG 建库不崩（`383335a`）+ `c1eaf6d23b07` downgrade 标不可逆（`38a494f`）/ 扣分 `uq_demerit_source` 改部分唯一索引、罚扫撤销后能再判（`72b411b`）/ `__version__` 去 `-v1-skeleton` 后缀（`ddc070b`）。
- **后端点呼判定时刻恒用 `server_now`，彻底不采纳客户端 `ts_local`**（7-06 拍板落地、spec 已定稿代码跟上，附 95 行测试，`60b66c2`）。
- iOS 审查修复批（CB/CC/NET/CON/DC/JP 系列 13 commit）：表单校验（夜学習 end<=start 行内提示 / 外出日下限 / 欠席日 JST 锚定 / 免餐終了日 min>max 防崩 / 変更届出寮日改过去校验）+ 网络失败三态（宅配/通知中心显失败态与骨架、不再假空态）+ 点呼上报失败 4 分支分流与 401 竞态 `tokenAtStart` 守卫 + 契約書图片解码压缩挪出主线程 + BusList 次便解析兜底 + 差戻→差し戻し全文统一等（`fba97ef`~`459f894` 系列）。
- iOS 上架命门：点呼入口生产占位「近日公開」+ 隐藏未实装邮箱登录 tab（spec v1.0 §2.2，`3e7fdc5`）。
- 公开门面与脱敏：LICENSE 去过时 Private 表述（`762dcb5`）/ 采购截图 24 张真正取消跟踪 — 补 `b2e895d` 声称但未生效的动作（`5aa3596`）。

### Added（文档 / 测试 / CI — 均非产品新功能）
- 运维文档群 E1：生产故障对应手册 13 场景（`09ef2ce`）/ 纸质点呼降级方案（`d010a52`）/ 监控告警清单 P0 五项（`15c1c97`）/ 备份恢复演练 SOP（`f83ec77`）。
- 合规文档群 E2：隐私政策补齐数据类别/保管期限/删除条款/未成年人（`31e9784`）/ 未成年人知情同意流程包（`2a645bb`）/ 利用規約日语单页 terms.html 起草 — 未签收未部署（`3ae169d`）。
- 测试：iOS C2 高杠杆 12 条（`e7c637e`）+ Android C2 8 条并抽出 `data/{rollcall,account,format}` 纯逻辑层（`881aa52`）/ 后端迁移 smoke 三层回归 + 字段集快照 B-中-17 + 女寮对称用例（`8cca8c0` / `0018f22` / `8c0ec2f`）。
- CI：后端起 postgres 容器真跑迁移链 — 假绿#1 全闭环（`c313d9b`）/ iOS 双 scheme + Android 单测/assembleDebug 两条工作流，push 前不生效（`1af9947`）。

### Changed
- コミュニティ管理页冷冻：老师网页菜单摘除防空头承诺、路由/组件/songs 接口保留（7-06 拍板 C，`f66b812`）+ 设计档冷冻登记（`c8c8c18`）。
- 契约/spec 治理：点呼判定时间基准两层定稿 — 过渡态 server_now / 点呼机波 swipe_time（`4a891e7`）/ API_CONVENTIONS §1 包络实装状态标注 + RollCall_Spec G2 残留口径连锁 6 处对齐 6-09 阶段发布（`29cc7e8`）/ spec 路径 B nonce 旧流程改写为现行 ST25DV 邮箱设计（`a4e9e41`）/ 点呼机 main.py 占位注释清 6-02 废弃架构（`8a86819`）。
- 文档大治理：7 份各端 README 按 7-10 盘点大修（`dc24a9f`）/ 根 README 死链修复 + 五端架构 mermaid 图（`dadb9f1`）/ DEPLOY.md 审核员账号对齐 seed 现实（`dbab96c`）/ .env.example SendGrid→Resend（`979fe9c`）/ 三端设计日志补 7-13 执行批（`70c685c`）/ 6-17 审查报告引用改指 archive（`cd182cf`）/ 4 份过期文件移出公开树（`6b1574f`）。

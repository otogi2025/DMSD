...
> - `.claude/skills/project-overview/SKILL.md` = 1450 个文件逐个的「字典」，想知道**某个具体文件干嘛**时查。
>
...
1. **响应格式**：⚠️ **2026-06-10 全量审查证实文档与代码不符** —— `API_CONVENTIONS.md` 原写「成功 `{ok:true, data:{}}`；失败 `{ok:false, error:{...}}`」，但后端 29 个路由实际全部直接返回数据模型（没有 `{ok,data}` 包络），iOS 也直接解码，两侧代码互相一致、能跑通。这个包络契约**没有任何实现**。改文档对齐现实（推荐，零代码改动）还是真去实现包络（两侧大改，不建议）待 itsuki 拍板——**新代码先照现实写（直接返回模型）**。详见 `00_admin/项目心智模型.md §3/§6`。
...
改任何端，下面这些都不能破（来源 `flow_design.md` + `decisions/decision_log.md`）：
...
完整决策脉络在 `05_logs/decisions/decision_log.md`。下面是理解项目最该知道的几条——它们也是 AC 叙事的核心：
...
3. `CHANGELOG.md` 顶部「全版本一览」—— 一行一版本的编年史（原 progress_overview.md 已退役，职能由它 + 本文件 §8.5 取代）。
...
| 6 | `05_logs/decisions/decision_log.md` | 关键决策为什么这样定 |
...
| `05_logs/decisions/decision_log.md` | 重大决策脉络：之前怎么定的 / 新决定 / 为什么改 / 影响什么 | 所有想理解项目「为什么长这样」的人 |
| `00_admin/WIP.md` | 当下书签：最近 5 次会话 + 当前焦点 + 多会话占用 | AI 每次开局必读 |
| `00_admin/TODO.md` | 全部未完成事项的完整清单（唯一真值） | itsuki + AI |

---

**最后更新**：2026-06-10 晚（对账校准 — §5.1 响应格式标「与代码不符」对齐心智模型 §3（{ok,data} 包络两侧代码都没实现）+ `decision_log.md` 路径全改 `decisions/decision_log.md`（6-10 移入子目录）+ 文件字典数 1396→1450）。上一轮：2026-06-10（补「发给外部 AI 当老师」使用场景 — 加 §4.5 核心规则数字速查 / §8.5 演化时间线 / §15 AI 协作体系；表数 34→36、路由 25→27 校准到当日快照）
